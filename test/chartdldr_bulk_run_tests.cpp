...
TEST(ChartDldrBulkSessionPolicy, ModeUiMatrix) {
  ...
  EXPECT_FALSE(scheduled_hidden.ConfirmBeforeStart());
  EXPECT_TRUE(scheduled_hidden.SkipManualCharts());
  EXPECT_TRUE(scheduled_hidden.AllowEmptySelection());
  EXPECT_TRUE(scheduled_hidden.ForceFullSelection());
  EXPECT_FALSE(scheduled_hidden.BindSelectedCatalogOnly());
  ...
  EXPECT_FALSE(interactive.SkipManualCharts());
  EXPECT_TRUE(interactive.ConfirmBeforeStart());
  EXPECT_FALSE(interactive.PreserveChartSelection());
  EXPECT_FALSE(interactive.BindSelectedCatalogOnly());
  EXPECT_TRUE(interactive.FocusChartsAfter());
  EXPECT_FALSE(interactive.ForceFullSelection());
  ...
  EXPECT_TRUE(manual.materialize);
  EXPECT_TRUE(manual.focus_charts_after);
