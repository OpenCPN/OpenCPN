...
TEST(NavUtils, fromDMM) {
  ...
  std::vector<TestCase> test_cases = {
      // @todo Test fails, returning 4927, which is not a valid co-ordinate.
      // Code should return an error instead.
      // {"4916.45N", 49.27416666666667},
      // @todo Test fails, returning 12311, which is not a valid co-ordinate.
      // Code should return an error instead.
      // {"12311.12W", -123.18533333333333},
      // @todo Test fails, returning 9000, which is not a valid co-ordinate.
      // Code should return an error instead.
      // {"9000.00S", -90.0},
      // @todo Test fails, returning 18000, which is not a valid co-ordinate.
      // Code should return an error instead.
      //{"18000.00E", 180.0},
      // @todo Test fails, returning 4530, which is not a valid co-ordinate.
      // Code should return an error instead.
      //{"04530.00W", -45.5},
      {"0000.00N", 0.0},          {"37°54.204' N", 37.9034},
      {"N37 54 12", 37.9034},     {"37°54'12\"", 37.9034},
      {"37.9034", 37.9034},       {"122°18.621' W", -122.31035},
      {"122w 18 37", -122.31027}, {"-122.31035", -122.31035},
      {"-122 18.37", -122.30616}, {"-122 18 37", -122.31027},
  };
...
TEST(NavUtils, toSDMM) {
  ...
  std::vector<TestCase> test_cases = {
      ...
      {{0, -42.42, true},
       "-42\xB0 25.2000'"},  // @todo Fix code to produce correct output, see
                             // https://github.com/OpenCPN/OpenCPN/pull/4851 -
                             // today it produces a garbage value of "-42°
                             // 5065.2000'"
      {{0, -122.31035, true},
       "-122\xB0 18.6210'"},  // @todo Fix code to produce correct output, see
                              // https://github.com/OpenCPN/OpenCPN/pull/4851 -
                              // today it produces a garbage value of "-122\xB0
                              // 14658.6210'"
  ...
  for (const auto& test_case : test_cases) {
    wxString result = toSDMM(test_case.input.NEflag, test_case.input.degrees,
                             test_case.input.hi_precision)
                          .ToStdString();
