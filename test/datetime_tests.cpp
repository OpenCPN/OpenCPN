...
TEST_F(DateTimeFormatTest, LocalTimezoneEST) {
  ...
  EXPECT_TRUE(result.Contains(" EST")) << "Actual timezone: " << result;

  // Test 2: request with default date/time format.
  // This should use the default format string which is $weekday_short_date_time
  opts = DateTimeFormatOptions().SetTimezone("Local Time");
  result = ocpn::toUsrDateTimeFormat(testDate, opts, us_locale);
  EXPECT_EQ(result, "Wed 02/22/2023 07:45:57 AM EST")
      << "Actual date/time: " << result;
  ...
  // Test 5: request to format in Local Time with $hour_minutes_seconds format.
  opts = DateTimeFormatOptions()
             .SetFormatString("$hour_minutes_seconds")
             .SetTimezone("Local Time");
  result = ocpn::toUsrDateTimeFormat(testDate, opts, us_locale);
  EXPECT_EQ(result, "07:45:57 AM EST")
      << "Actual date/time: '" << result << "'";
}
...
TEST_F(DateTimeFormatTest, LocalTimezoneCETSwedish) {
  ...
  // sommartid" text Instead, it should use the standardized "CEST" abbreviation
  EXPECT_TRUE(result.Contains(" CET")) << "Actual timezone: " << result;
  ...
  EXPECT_TRUE(result.Contains("05:15:31 PM CEST") ||
              result.Contains("05:15:31 PM"))
      << "Actual time portion: " << result;
  EXPECT_FALSE(result.Contains("sommartid"))
      << "Should not contain 'sommartid' suffix: " << result;

  // Test 4: Non-DST date in CET
  wxDateTime testDateWinter(21, wxDateTime::Jan, 2024, 15, 15, 31);
  testDateWinter.MakeFromTimezone(wxDateTime::UTC);
  opts = DateTimeFormatOptions()
             .SetFormatString("%Y-%m-%d %H:%M:%S")
             .SetTimezone("Local Time");
  result = ocpn::toUsrDateTimeFormat(testDateWinter, opts, locale);

  // Should use CET (not CEST) for winter time
  EXPECT_TRUE(result.Contains(" CET"))
      << "Winter timezone should be CET: " << result;
  EXPECT_FALSE(result.Contains("sommartid"))
      << "Winter time should not mention 'sommartid': " << result;
}