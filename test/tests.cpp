...
TEST_F(TToStringTest, LMTTimeZoneOneHourEast) {
  ...
  wxString result = ocpn::TToString(testDate, "%A, %B %d, %Y %I:%M:%S %p", DT_TZ_LMT, DT_TZ_UTC, 15.0); // 15 degrees East
...
TEST_F(TToStringTest, LMTTimeZone30MinEast) {
  wxDateTime testDate(22, wxDateTime::Jan, 2023, 7, 0, 0);
  testDate.MakeFromTimezone(wxDateTime::UTC);
  wxString result = ocpn::TToString(testDate, "%A, %B %d, %Y %I:%M:%S %p", DT_TZ_LMT, DT_TZ_UTC, 7.5); // 7.5 degrees East
...
TEST_F(TToStringTest, LMTTimeZoneWest) {
  wxDateTime testDate(22, wxDateTime::Jan, 2023, 7, 0, 0);
  testDate.MakeFromTimezone(wxDateTime::UTC);
  wxString result = ocpn::TToString(testDate, "%A, %B %d, %Y %I:%M:%S %p", DT_TZ_LMT, DT_TZ_UTC, -37.5); // 37.5 degrees West
...
TEST_F(TToStringTest, CustomFormatStringEnUS) {
  ...
  testDate.MakeFromTimezone(wxDateTime::UTC);
  wxString result = ocpn::TToString(testDate, "%A, %B %d, %Y %I:%M:%S %p", wxEmptyString, DT_TZ_UTC, NAN);
...
TEST_F(TToStringTest, CustomFormatStringUTC) {
  wxDateTime testDate(22, wxDateTime::Jan, 2023, 12, 45, 57);
  testDate.MakeFromTimezone(wxDateTime::UTC);
  wxString result = ocpn::TToString(testDate, "%Y-%m-%d %H:%M:%S", DT_TZ_UTC, DT_TZ_UTC, NAN);
...
TEST_F(TToStringTest, CustomTimeZone) {
  ...
  testDate.MakeFromTimezone(wxDateTime::UTC);
  wxString result = ocpn::TToString(testDate, "%Y-%m-%d %H:%M:%S", "EST", DT_TZ_UTC, NAN);
  // As of now, the function doesn't handle custom timezone strings, so it should default to UTC
