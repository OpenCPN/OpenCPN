...
make headless pairing using ssh. Prequisites:
...
Verify using curl to make a pin command. `api_key` is as of above, hemulen is
from step ①.  In my case:

    $ curl --insecure "https://raspbian:8443/api/ping?api_key=bc45def81515&source=hemulen"
