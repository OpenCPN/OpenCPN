...
3. Merge upstream changes using `git merge -X theirs upstream/latest`
...
8. Create a source package using `debuild -S`.
9. List all available source packages using `ls -lt ../*source.changes`.
10. Push the most recent package to a PPA using something like
