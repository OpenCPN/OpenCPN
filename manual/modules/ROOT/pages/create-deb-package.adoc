...
Here are three main tasks:
  ...
  - <<import-version,Importing new opencpn releases into the packaging
    branches>>.
  - <<src-release, Creating a new downstream release and upload it into an
    Ubuntu PPA>>
...
3. Do possible changes in the new release.
...
5. Finalize the changelog entry using `dch -r`. 
6. Commit the change in _debian/changelog_
7. Create a source package using `debuild -S`.
8. List all available source packages using `ls -lt ../*source.changes`.
9. Push the most recent package to a PPA using something like

     $ `dput ppa:leamas-alec/opencpn \
     ../opencpn_5.9.2-beta2+dfsg-1ubuntu1~bpo2204.8_source.changes`.
