...
enum { DS_ERROR_PORTNOTFOUND };

//      End-of-sentence types
enum { DS_EOS_CRLF, DS_EOS_LF, DS_EOS_CR };

//      Serial port handshake type
enum { DS_HANDSHAKE_NONE, DS_HANDSHAKE_XON_XOFF };
...
//              Serial:COM4
//              TCP:192.168.1.1:5200            (TCP source, address and port
//              specified) GPSD:192.168.2.3:5400           (GPSD Wire protocol,
...
protected:
  DataStream(wxEvtHandler *input_consumer, const ConnectionType conn_type,
             const wxString &Port, const wxString &BaudRate,
             dsPortType io_select, int priority, bool bGarmin, int EOS_type,
             int handshake_type);
...
protected:
  ...
  OCP_DataStreamInput_Thread *GetSecondaryThread() {
...
public:
  NullDataStream(wxEvtHandler *input_consumer, const ConnectionParams *params)
      : DataStream(input_consumer, params) {}
};
