 ...
 *         - "Local Time": Format date/time in the local time as configured in the system.
...
// For example, $short_date is resolved to "12/31/2021" in the US locale and "31/12/2021" in the UK locale.
...
const wxString DT_HOUR_MINUTES_SECONDS = "$hour_minutes_seconds"; // 15:34:56 zero-padded.
const wxString DT_HOUR_MINUTES = "$hour_minutes"; // 15:34 zero-padded.
const wxString DT_LONG_DATE_TIME = DT_LONG_DATE + " " + DT_HOUR_MINUTES_SECONDS; // Thursday December 31, 2021 12:34:56
const wxString DT_SHORT_DATE_TIME = DT_SHORT_DATE + " " + DT_HOUR_MINUTES_SECONDS; // 12/31/2021 15:34:56
const wxString DT_WEEKDAY_SHORT_DATE_TIME = DT_WEEKDAY_SHORT_DATE + " " + DT_HOUR_MINUTES_SECONDS; // Thu 12/31/2021 15:34:56
 ...
 * @param time_zone The timezone to use when formatting the date/time. Options include:
 *        - Empty string (default): the date/time is formatted according to the OpenCPN timezone settings.
 ...
 *
 * @note It is strongly recommended to use the default value (empty string) for the time_zone parameter
 *       to ensure consistency of the date/time representation across the entire application. This will
 *       format the date/time according to the OpenCPN timezone settings.
 */
extern DECL_EXP wxString TToString(
    const wxDateTime date_time,
    const wxString format_string = DT_WEEKDAY_SHORT_DATE_TIME,
    const wxString time_zone = wxEmptyString,
