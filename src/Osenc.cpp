...
const char *Osenc::GetFeatureAcronymFromTypecode( int typeCode)
{
    const char *acronym = NULL;
    if( g_csv_locn.Len() ) {
        wxString oc_file( g_csv_locn );
        oc_file.Append( _T("/s57objectclasses.csv") );
        char code[20];
        snprintf( code, 19, "%d", typeCode );
        acronym = MyCSVGetField( oc_file.mb_str(), "Code",                  // match field
                               code,                                          // match value
                               CC_ExactString, "Acronym" );                  // return field
    }
...
//                }
                ...
                const char *acronym = GetFeatureAcronymFromTypecode( featureTypeCode );
                
                if(acronym){
                    obj = new S57Obj(acronym);
                    obj->Index = featureID;
...
int Osenc::createSenc200(const wxString& FullPath000, const wxString& SENCFileName, bool b_showProg)
{
    ...
    if( !WriteHeaderRecord200( fps57, HEADER_CELL_NATIVESCALE, (uint16_t)m_native_scale) ){
        fclose( fps57 );
        return ERROR_SENCFILE_ABORT;
    }
...
bool Osenc::CreateLineFeatureGeometryRecord200( OGRFeature *pFeature, FILE *fpOut)
{
    OGRGeometry *pGeo = pFeature->GetGeometryRef();
...
bool Osenc::CreateAreaFeatureGeometryRecord200(OGRFeature *pFeature, FILE * fpOut)
{
    int error_code;
    ...
    if( ( pGeo != NULL ) ) {
        ...
        switch( gType ){
            ...
            case wkbLineString: {
                ...
                if( !CreateLineFeatureGeometryRecord200(pFeature, fpOut) )
                    return false;
            ...
            case wkbMultiPoint25D:{
                ...
                case wkbPolygon: {
                    ...
                    if( !CreateAreaFeatureGeometryRecord200(pFeature, fpOut) )
                        return false;
                }
