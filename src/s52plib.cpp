...
void DrawWuLine( wxDC *pDC, int X0, int Y0, int X1, int Y1, wxColour clrLine,
		int dash, int space );
...
bool s52plib::RenderHPGLtoDC( char *str, char *col, wxDC *pdc, wxPoint &r,
		wxPoint &pivot, double rot_angle ) {
	int width = 1;
	double radius = 0.0;
	int tessObj = FALSE;
	int polyMode = FALSE;
			...
			do {
				if( *str == '0' ) // start a new poly
					...
					INST ( CI )
						radius = (double) atoi( str );
						wxColour color( newColor->R, newColor->G, newColor->B );
						wxBrush *pthisbrush = wxTheBrushList->FindOrCreateBrush(
								color, wxSOLID );
						...
						wxLogMessage(
								_T ( "SEQuencer:_renderHPGL(): fixme AA instruction not implemented" ) );
						inBegEnd = FALSE;
					} // INST AA
					else {
						tessObj = TRUE;
...
// never called --not tested
			...
			if( tessObj ) {
				wxString msg =
						_T ( "SEQuencer:_renderHPGL(): fixme EP instruction not implemented " );
				LogMessageOnce( msg );
			}
...
// Symbol Call    --never used
			...
			{
				wxString msg =
						_T ( "SEQuencer:_renderHPGL(): fixme SC instruction not implemented " );
				LogMessageOnce( msg );
			}
		}
		++str;
...
bool s52plib::RenderHPGLtoGL( char *str, char *col, wxPoint &r, wxPoint &pivot,
		double rot_angle ) {
	int width = 1;
	double radius = 0.0;
//      int    tessObj       = FALSE;
			...
			if( seg_len >= 1.0 ) {
				if( seg_len <= sym_len * sym_factor ) {
					...
					while( s + ( sym_len * sym_factor ) < seg_len ) {
						...
						RenderHPGLtoDC( str, col, pdc, r, pivot,
								(double) tdeg );
...
void DrawWuLine( wxDC *pDC, int X0, int Y0, int X1, int Y1, wxColour clrLine,
	...
	double len = sqrt(
			pow( (double) ( X0 - X1 ), 2 ) + pow( (double) ( Y0 - Y1 ), 2 ) );

	int dot_cnt = dash;
	if( space == 0 ) dot_cnt = -1; // No spaces, dots run (almost) forever
	...
	if( DeltaY > DeltaX ) {
		...
		while( --DeltaY ) {
			...
			if( bdraw ) img.SetRGB( xp0, yp0, rr, gr, br );
			...
			rr = ( rb > rl ?
					( (unsigned char) ( ( (double) (
							grayl < grayb ? ( Weighting ^ 255 ) : Weighting ) )
							/ 255.0 * ( rb - rl ) + rl ) ) :
					( (unsigned char) ( ( (double) (
							grayl < grayb ? ( Weighting ^ 255 ) : Weighting ) )
							/ 255.0 * ( rl - rb ) + rb ) ) );
			...
			if( bdraw ) img.SetRGB( xp0 + XDir, yp0, rr, gr, br );
...
//                convert from image to bitmap, then blit it back into pDC
	...
	while( --DeltaX ) {
		...
		unsigned char rr =
				( rb > rl ?
						( (unsigned char) ( ( (double) (
								grayl < grayb ? Weighting : ( Weighting ^ 255 ) ) )
								/ 255.0 * ( rb - rl ) + rl ) ) :
						( (unsigned char) ( ( (double) (
								grayl < grayb ? Weighting : ( Weighting ^ 255 ) ) )
								/ 255.0 * ( rl - rb ) + rb ) ) );
		...
		if( bdraw ) img.SetRGB( xp0, yp0 + 1, rr, gr, br );
