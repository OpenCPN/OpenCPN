...
//         mdns_query_answer_unicast(int sock, const void* address, size_t address_size, void* buffer, size_t capacity,
//                                   uint16_t query_id, mdns_record_type_t record_type,
...
void service_mdns(const char* hostname, const char* service_name, int service_port) {
  ...
  hostname_qualified_string.length = strlen(qualified_hostname_buffer);
	...
	// "<hostname>.<_service-name>._tcp.local."
//   mdns_record_t r1 = {.name = service.service,
