...
static void chk_fread (void *ptr, size_t size, size_t nmemb, FILE *stream) {
    ...
    if (ret != nmemb) {
        LOG_DEBUG ( "libtcd unexpected error: fread failed\n");
        LOG_DEBUG ( "nmemb = %lu, got %lu\n", nmemb, ret);
        abort();
    }
}
...
static void chk_fwrite (const void *ptr, size_t size, size_t nmemb,
    ...
    if (ret != nmemb) {
        LOG_DEBUG ("libtcd unexpected error: fwrite failed\n");
        LOG_DEBUG ("nmemb = %lu, got %lu\n", nmemb, ret);
        LOG_DEBUG ("The database is probably corrupt now.\n");
        abort();
    }
}
...
static void boundscheck_monologue (const NV_CHAR *string) {
    ...
    if (strlen(string) >= MONOLOGUE_LENGTH) {
        LOG_DEBUG("libtcd fatal error:  static buffer size exceeded\n");
        LOG_DEBUG("Buffer is size MONOLOGUE_LENGTH (%u)\n",
                 MONOLOGUE_LENGTH);
        LOG_DEBUG("String is length %lu\n", strlen(string));
        LOG_DEBUG("The offending string is:\n%s\n", string);
        exit (-1);
    }
}
...
static void boundscheck_oneliner (const NV_CHAR *string) {
    ...
    if (strlen(string) >= ONELINER_LENGTH) {
        LOG_DEBUG("libtcd fatal error:  static buffer size exceeded\n");
        LOG_DEBUG("Buffer is size ONELINER_LENGTH (%u)\n",
                 ONELINER_LENGTH);
        LOG_DEBUG("String is length %lu\n", strlen(string));
        LOG_DEBUG("The offending string is:\n%s\n", string);
        exit (-1);
