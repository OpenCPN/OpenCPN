...
#ifdef ocpnUSE_SVG
static void rsvg_cairo_size_callback( int *width, int *height, void *data )
{
...
static void log_svg_error( GError* err )
{
    if (err) {
        wxLogMessage( wxString( err->message ) );
        g_error_free( err );
    }
}
...
static wxBitmap LoadSVG( const wxString filename, unsigned int width, unsigned int height )
{
    ...
    else
    {
        ...
        cairo_matrix_init_scale( &cm, (double)width / dimensions.width, (double)height / dimensions.height );
