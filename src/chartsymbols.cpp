...
bool ChartSymbols::LoadConfigFile( s52plib* plibArg, wxString s52ilePath ) {
	...
	if( ! wxFileName::FileExists(fullFilePath) ) {
		wxLogWarning( _("ChartSymbols::LoadConfigFile(): File not found: %s"), fullFilePath );
	}
	...
	if( ! doc.Load( fullFilePath ) ) {
		wxLogError( _("ChartSymbols::LoadConfigFile(): Failed to load %s"), fullFilePath );
		return false;
	}

	wxLogMessage( _("ChartSymbols loaded from %s"), fullFilePath );
...
int ChartSymbols::LoadRasterFileForColorTable( int tableNo ) {
	...
	wxLogError( _("ChartSymbols::LoadRasterFileForColorTable(): Failed to load raster symbols file (%s)."), filename );
	return false;
}
