...
static std::string do_readlink(const char* link) {
    ...
    if (r == -1 && errno == EINVAL) {
        ...
        wxLogWarning("Cannot run readlink on %s: %s", path, strerror(errno));
        return path;
    }
    if (*target == '/') {
        return target;     // link is absolute
    }
