...
int RazdsParser::ReadS52Line( char *pBuffer, const char *delim, int nCount,
		FILE *fp ) {
	int ret;
...
int RazdsParser::ParsePos( position *pos, char *buf, bool patt ) {
	...
	sscanf( buf, "%5d%5d%5d%5d%5d%5d", &pos->pivot_x.PACL, &pos->pivot_y.PARW,
			&pos->bnbox_w.PAHL, &pos->bnbox_h.PAVL, &pos->bnbox_x.PBXC,
			&pos->bnbox_y.PBXR );
	return 1;
}
...
int RazdsParser::ParseLBID( FILE *fp ) {
	...
	if( token.ToDouble( &version ) ) {
		...
		plib->m_VersionMinor =
				(int) floor( ( ( version - plib->m_VersionMajor ) * 10 ) + 0.5 );
	} else {
...
int RazdsParser::ParseLNST( FILE *fp ) {
	...
	do {
		...
		MOD_REC ( LXPO ) lnst->exposition.LXPO->Append(
				wxString( pBuf + 9, wxConvUTF8 ) );
		...
		MOD_REC ( **** ) {
			...
			else if( lnst->name.LINM != lnstmp->name.LINM ) ( *plib->_line_sym )[key] =
					lnst;
			else
			assert( 0 );
...
int RazdsParser::ParsePATT( FILE *fp ) {
	...
	do {
		...
		MOD_REC ( PXPO ) patt->exposition.PXPO->Append(
				wxString( pBuf + 9, wxConvUTF8 ) );
...
int RazdsParser::ParseSYMB( FILE *fp, RuleHash *pHash ) {
	...
	do {
		...
		MOD_REC ( SXPO ) symb->exposition.SXPO->Append(
				wxString( pBuf + 9, wxConvUTF8 ) );

		MOD_REC ( SBTM ) {
			bitmap_width = symb->pos.symb.bnbox_w.SYHL;
			if( bitmap_width > 200 ) wxLogMessage(
					_T ( "ParseSymb....bitmap too wide." ) );
			strncpy( pbm_line, pBuf + 9, bitmap_width );
			pbm_line[bitmap_width] = 0;
...
int RazdsParser::LoadFile( s52plib* plibArg, const wxString& PLib ) {
	...
	if( fp == NULL ) {
		wxLogError( _("   S52PLIB: Cannot open S52 rules file: %s"), PLib );
		return 0;
	}
