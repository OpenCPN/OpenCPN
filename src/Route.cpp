...
void Route::DrawPointWhich(ocpnDC &dc, ChartCanvas *canvas, int iPoint,
                           wxPoint *rpn) {
  if (iPoint <= GetnPoints()) GetPoint(iPoint)->Draw(dc, canvas, rpn);
}

void Route::DrawSegment(ocpnDC &dc, ChartCanvas *canvas, wxPoint *rp1,
...
void Route::Draw(ocpnDC &dc, ChartCanvas *canvas, const LLBBox &box) {
  ...
  wxPoint rpt1, rpt2;
  if (m_bVisible) DrawPointWhich(dc, canvas, 1, &rpt1);

  wxRoutePointListNode *node = pRoutePointList->GetFirst();
  RoutePoint *prp1 = node->GetData();
  node = node->GetNext();

  if (m_bVisible || prp1->IsShared()) prp1->Draw(dc, canvas, NULL);
  ...
  while (node) {
    ...
    if (m_bVisible || prp2->IsShared()) prp2->Draw(dc, canvas, &rpt2);
...
void Route::DrawGLLines(ViewPort &vp, ocpnDC *dc, ChartCanvas *canvas) {
  ...
  if (GetnPoints() == 1 &&
      dc) {  // single point.. make sure it shows up for highlighting
...
void Route::DrawGLRouteLines(ViewPort &vp, ChartCanvas *canvas) {
  ...
  if (m_width != wxPENSTYLE_INVALID) width = m_width;
...
void Route::RenderSegment(ocpnDC &dc, int xa, int ya, int xb, int yb,
...
void Route::RenderSegmentArrowsGL(ocpnDC &dc, int xa, int ya, int xb, int yb,
                                  ViewPort &vp) {
#ifdef ocpnUSE_GL
  //    Draw a direction arrow
...
void Route::CalculateDCRect(wxDC &dc_route, ChartCanvas *canvas,
                            wxRect *prect) {
  ...
  if (m_bVisible) {
    ...
    while (node) {
      ...
      r.Inflate(
          m_hiliteWidth,
          m_hiliteWidth);  // allow for large hilite circles at segment ends
...
int Route::SendToGPS(const wxString &com_name, bool bsend_waypoints,
                     SendToGpsDlg *dialog) {
  int result = 0;
