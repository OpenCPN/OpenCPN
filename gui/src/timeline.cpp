...
Timeline::Timeline(wxWindow* parent, const wxDateTime& timelineStartTs,
  ...
  m_autoHideDelay = 3000;  // 3 second auto-hide delay
...
void Timeline::OnNowButton(wxCommandEvent& event) {
  ...
  if (m_isTrackingNow) {
    ...
    // Update button appearance to indicate tracking mode
    wxString iconDir = g_Platform->GetSharedDataDir() + "uidata/timeline/";
    m_nowButton->SetBitmapLabel(
        LoadScaledSVG(iconDir + "now_tracking.svg", 32, 32));
