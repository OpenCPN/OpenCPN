...
/**
 * Check if a line between two points crosses land using distance field approach.
 * This implementation is a replacement for the original polygon-intersection method
 * and delivers near-constant time performance for any number of coastline polygons.
 * 
 ...
 */
bool ShapeBaseChart::CrossesLandWithDistanceField(double &lat1, double &lon1, double &lat2, double &lon2) {
  if (!_is_usable) {
      return false;
  }
  if (!_reader && !_loading) {
  ...
  return dfManager.CrossesLand(lat1, lon1, lat2, lon2, relevantTiles, _tiles, _reader);
}
