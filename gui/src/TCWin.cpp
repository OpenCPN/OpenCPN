...
private:
  void OnPaint(wxPaintEvent &event) {
    ...
    // Create chart rectangle with proper left margin
  ...
  void OnMouseMove(wxMouseEvent &event) {
    // Get mouse position relative to this chart panel
    wxPoint panelPos = event.GetPosition();

    // Convert to main window coordinates for compatibility with existing code
    wxPoint mainWindowPos = panelPos + GetPosition();

    // Forward to parent's mouse handling
...
TCWin::TCWin(ChartCanvas *parent, int x, int y, void *pvIDX) {
  ...
  // Create the layout using sizers
  CreateLayout();

  // Force initial layout calculation
  Layout();

  // Initialize graph rectangle - will be set properly by
  // TideChartPanel::OnPaint() Use a default rectangle that will be overridden
  // during painting
  m_graph_rect = wxRect(0, 0, 400, 200);

  //  Measure the size of a generic button for calculations
...
void TCWin::OnPaint(wxPaintEvent &event) {
  ...
  // manual positioning calculations here.
}
