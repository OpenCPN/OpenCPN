...
void MyFrame::ToggleTimeline() {
  ...
  if (g_bShowTimeline) {
    ...
    // Hide and optionally destroy timeline
    if (m_pTimeline) {
...
// Global timeline access function for plugin API
...
void MyFrame::RegisterGlobalMenuItems() {
  ...
  view_menu->AppendCheckItem(ID_MENU_UI_TIMELINE,
                             _menuText(_("Show Timeline"), _T("Ctrl-T")));
...
void MyFrame::OnEvtPlugInMessage(OCPN_MsgEvent &event) {
  ...
  } else if (message_ID == _T("OCPN_TRACK_REQUEST")) {
    wxJSONValue root;
    wxJSONReader reader;
