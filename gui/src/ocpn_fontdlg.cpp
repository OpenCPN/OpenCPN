...
#if 1 // wxUSE_FONTDLG && (defined(__WXGTK__) || defined(__WXGPE__) || defined(__WXUNIVERSAL__))
...
{
public:
    OCPNFontPreviewer(wxWindow *parent, const wxSize& sz = wxDefaultSize) : wxWindow(parent, wxID_ANY, wxDefaultPosition, sz)
    {
    }
...
wxBEGIN_EVENT_TABLE(OCPNFontPreviewer, wxWindow)
    EVT_PAINT(OCPNFontPreviewer::OnPaint)
wxEND_EVENT_TABLE()
...
wxBEGIN_EVENT_TABLE(ocpnGenericFontDialog, wxDialog)
    ...
    EVT_CHOICE(wxID_FONT_FAMILY, ocpnGenericFontDialog::OnChangeFont)
    EVT_CHOICE(wxID_FONT_COLOUR, ocpnGenericFontDialog::OnChangeFont)
#if USE_SPINCTRL_FOR_POINT_SIZE
    EVT_SPINCTRL(wxID_FONT_SIZE, ocpnGenericFontDialog::OnChangeSize)
    EVT_TEXT(wxID_FONT_SIZE, ocpnGenericFontDialog::OnChangeFont)
#else
    EVT_CHOICE(wxID_FONT_SIZE, ocpnGenericFontDialog::OnChangeFont)
#endif
...
#define NUM_COLS 48
static wxString ocpnColourDialogNames[NUM_COLS]={wxT("ORANGE"),
                    wxT("GOLDENROD"),
                    wxT("WHEAT"),
                    wxT("SPRING GREEN"),
                    wxT("SKY BLUE"),
...
//#endif
    ...
    if (m_fontData.GetEnableEffects())
    {
        ...
        m_colourChoice = new wxChoice( this, wxID_FONT_COLOUR, wxDefaultPosition, colourSize, NUM_COLS, ocpnColourDialogNames, 0 );
    ...
    m_familyChoice->SetStringSelection( ocpnFontFamilyIntToString(m_dialogFont.GetFamily()) );
    m_styleChoice->SetStringSelection(ocpnFontStyleIntToString(m_dialogFont.GetStyle()));
    m_weightChoice->SetStringSelection(ocpnFontWeightIntToString(m_dialogFont.GetWeight()));
...
void ocpnGenericFontDialog::DoChangeFont()
{
    ...
    wxFontFamily fontFamily = ocpnFontFamilyStringToInt(m_familyChoice->GetStringSelection());
    wxFontWeight fontWeight = ocpnFontWeightStringToInt(m_weightChoice->GetStringSelection());
    wxFontStyle fontStyle = ocpnFontStyleStringToInt(m_styleChoice->GetStringSelection());
#if USE_SPINCTRL_FOR_POINT_SIZE
