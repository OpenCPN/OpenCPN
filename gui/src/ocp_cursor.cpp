...
ocpCursorRefData::~ocpCursorRefData() {
  if (m_cursor) XFreeCursor((Display*)m_display, (Cursor)m_cursor);
}
...
ocpCursor::ocpCursor(const wxString& cursorName, long type, int hotSpotX,
                     int hotSpotY)
  ...
  //    Make a pixmap
  Pixmap cpixmap =
      XCreatePixmap((Display*)wxGlobalDisplay(), xroot, width, height, 1);

  //    Make an Ximage
  XImage* data_image = XCreateImage((Display*)wxGlobalDisplay(), xvisual, 1,
  ...
  if (cImage.HasMask()) {
    XImage* mask_image = XCreateImage((Display*)wxGlobalDisplay(), xvisual, 1,
                                      ZPixmap, 0, 0, width, height, 32, 0);
    mask_image->data =
    ...
    XPutImage((Display*)wxGlobalDisplay(), cmask, gc, mask_image, 0, 0, 0, 0,
              width, height);
  ...
  XPutImage((Display*)wxGlobalDisplay(), cpixmap, gc, data_image, 0, 0, 0, 0,
            width, height);
  ...
  M_CURSORDATA->m_cursor =
      (WXCursor)XCreatePixmapCursor((Display*)wxGlobalDisplay(), cpixmap, cmask,
                                    &fg, &bg, hotSpotX, hotSpotY);
}

ocpCursor::ocpCursor(const char** xpm_data, long type, int hotSpotX,
                     int hotSpotY)
    : wxCursor(wxCURSOR_CROSS) {
...

{
  ...
  //      wxMSW Bug???
  //      On Windows XP, conversion from wxImage to wxBitmap fails at the
  //      ::CreateDIBitmap() call unless a "compatible" dc is provided.  Why??
  ...
  //        HCURSOR hcursor = wxBitmapToHCURSOR ( wxBitmap ( cImage, ( wxDC & )
  //        dwxdc ),
  //                                              hotSpotX, hotSpotY );
...
//----------------------------------------------------------------------------------------------
//      A new constructor taking a static char ** of XPM data and assign as a
//      cursor
//----------------------------------------------------------------------------------------------
