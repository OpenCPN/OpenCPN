...
void DistanceField::InitializeGrid() {
  ...
  _highResGrid.resize(_highResGridSize);
  for (int i = 0; i < _highResGridSize; ++i) {
    _highResGrid[i].resize(_highResGridSize);
  }

  // Initialize medium-resolution grid
  _mediumResGrid.resize(_mediumResGridSize);
  for (int i = 0; i < _mediumResGridSize; ++i) {
    _mediumResGrid[i].resize(_mediumResGridSize);
  }

  // Initialize low-resolution grid
  _lowResGrid.resize(_lowResGridSize);
  for (int i = 0; i < _lowResGridSize; ++i) {
    _lowResGrid[i].resize(_lowResGridSize);
  }
}
  ...
  for (size_t featureIndex : featureIndices) {
    ...
    for (auto& ring : polygon->getRings()) {
      for (auto& point : ring.getPoints()) {
        ...
        // Convert lat/lon to grid indices for all resolution levels
        auto [highI, highJ, _] = LatLonToGrid(lat, lon);
        ...
        // Also mark in medium and low resolution grids
        int medI = static_cast<int>((lat - _minLat) / _mediumResCellSize);
        int medJ = static_cast<int>((lon - _minLon) / _mediumResCellSize);
        ...
        int lowI = static_cast<int>((lat - _minLat) / _lowResCellSize);
        int lowJ = static_cast<int>((lon - _minLon) / _lowResCellSize);
...
double DistanceField::QueryDistance(double lat, double lon) const {
  ...
  auto [lowI, lowJ, _] = LatLonToGrid(lat, lon);
  if (IsValidIndex(lowI, lowJ, ResolutionLevel::LOW)) {
  ...
  if (distance <= MEDIUM_RES_THRESHOLD) {
    ...
    auto [medI, medJ, __] = LatLonToGrid(lat, lon);
    if (IsValidIndex(medI, medJ, ResolutionLevel::MEDIUM)) {
    ...
    if (distance <= HIGH_RES_THRESHOLD) {
      ...
      auto [highI, highJ, ___] = LatLonToGrid(lat, lon);
      if (IsValidIndex(highI, highJ, ResolutionLevel::HIGH)) {
...
std::tuple<int, int, ResolutionLevel> DistanceField::LatLonToGrid(
    double lat, double lon) const {
  // Determine the resolution level based on point coordinates
  ResolutionLevel level;
  int i, j;
...
bool DistanceField::SaveToDisk(const std::string& baseDir) const {
  ...
  // Write low-resolution grid (compressed)
  for (int i = 0; i < _lowResGridSize; ++i) {
    for (int j = 0; j < _lowResGridSize; ++j) {
  ...
  // Write medium-resolution grid (compressed)
  for (int i = 0; i < _mediumResGridSize; ++i) {
    for (int j = 0; j < _mediumResGridSize; ++j) {
  ...
  // Write high-resolution grid (compressed)
  for (int i = 0; i < _highResGridSize; ++i) {
    for (int j = 0; j < _highResGridSize; ++j) {
...
bool DistanceField::LoadFromDisk(const std::string& baseDir) {
  ...
  file.close();
  _valid = true;
  MESSAGE_LOG << "Distance field loaded from " << fileName;
  return true;
}
