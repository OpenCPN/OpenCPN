...
/**
 * Represents an index entry for tidal and current data. Stores various attributes
 * and data related to tidal and current stations, including location, offsets, and harmonic constituents.
 */
class IDX_entry {
public:
  IDX_entry();
  ...
  source_data_t source_data_type;     ///< Format of the source data (ASCII or binary)
  TCDataSource *pDataSource;          ///< Pointer to the associated data source
  char source_ident[MAXNAMELEN];      ///< Identifier of the source (typically file name)
  ...
  /**
   * @brief Offset in seconds to convert from harmonic data (epochs) to the station time zone
   *
  ...
  time_t recent_highlow_calc_time;    ///< Timestamp of the most recent high/low calculation
