...
/**
 * A latitude/longitude key for 1x1 or 10x10 degree grid cells.
 * Used for indexing geographical data in a grid-based system where each cell
 * represents a 1x1 degree area. The lat/lon values represent integer floor
 * values of coordinates, effectively the southwest (bottom-left) corner of each
 * grid cell. For example, a coordinate of (45.7, -120.3) would be in the cell
 * with LatLonKey(45, -121). The system handles longitude wraparound at the
...
public:
   ...
   * @param lat Integer latitude value indicating the south edge of a 1-degree
   * latitude band.
   * @param lon Integer longitude value indicating the west edge of a 1-degree
   * longitude band.
   */
  ...
  /**
   * Integer latitude value representing the southern boundary of a 1-degree
   * latitude band. For example, lat=45 represents the area between 45°N and
   * 46°N.
   */
  int lat;
  /**
   * Integer longitude value representing the western boundary of a 1-degree
   * longitude band. For example, lon=-120 represents the area between 120°W and
   * 119°W. Handles wraparound at the international date line.
   */
  int lon;
