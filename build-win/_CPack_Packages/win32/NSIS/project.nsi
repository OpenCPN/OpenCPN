...
CMake variables
	...
	CPACK_PACKAGE_ICON "${CMAKE_SOURCE_DIR}/src/bitmaps\\\\opencpn.ico"
...
;# You must define these values
	...
	!define CPACK_PACKAGE_ICON "${CMAKE_SOURCE_DIR}/src/bitmaps/opencpn.ico"
	!define CPACK_BUILDWIN_DIR "${CMAKE_SOURCE_DIR}/buildwin"
	!define CPACK_NSIS_DIR "${CMAKE_SOURCE_DIR}/buildwin/NSIS_Unicode"
	*/
	!define /date YEAR "%Y"
	!define OUTPUT_FILE_NAME "opencpn_3.3.2303_setup.exe"
	!define INST_DIR "P:/working area/opencpn/remote/jongough/OpenCPN/build-win/_CPack_Packages/win32/NSIS/opencpn_3.3.2303_setup"
	...
	!define REG_KEY_UN "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenCPN 3.3.2303"
...
;--------------------------------
;# Includes
	!addincludedir "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode/Include"
	!addplugindir "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode/Plugins"
	...
	!define MUI_UI "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode/UI/opencpn_ui.exe"

;--------------------------------
;# General
  ;# Name and file
  Name "OpenCPN 3.3.2303"
  OutFile "P:/working area/opencpn/remote/jongough/OpenCPN/build-win/_CPack_Packages/win32/NSIS/${OUTPUT_FILE_NAME}"
...
;# Interface Settings
  ...
  ;# MUI Settings / Header
	!define MUI_ICON "P:/working area/opencpn/remote/jongough/OpenCPN/src/bitmaps\opencpn.ico"
    !define MUI_HEADERIMAGE
    !define MUI_HEADERIMAGE_LEFT
    !define MUI_HEADERIMAGE_BITMAP "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode\Bitmaps\header-install.bmp"
    !define MUI_HEADERIMAGE_UNBITMAP "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode\Bitmaps\header-uninstall.bmp"

  ;# MUI Settings / Wizard
    !define MUI_WELCOMEFINISHPAGE_BITMAP "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode\Bitmaps\wizard-install.bmp"
    !define MUI_UNWELCOMEFINISHPAGE_BITMAP "P:/working area/opencpn/remote/jongough/OpenCPN/buildwin/NSIS_Unicode\Bitmaps\wizard-uninstall.bmp"
...
*/
	...
	;!insertmacro MUI_PAGE_LICENSE "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
...
SectionGroup "!Reset Config Section Group" SecGrpResetConfig
	Section /o "Delete Config File Section" SecDeleteConfigFile
		...
		SetShellVarContext all
		${If} ${FileExists} "$APPDATA\opencpn\opencpn.ini"
			Delete "$APPDATA\opencpn\opencpn.ini"
			ClearErrors
		${EndIf}
	SectionEnd
	...
	Section /o "Delete ChartDB Section" SecDeleteChartDB
		...
		SetShellVarContext all
		${If} ${FileExists} "$APPDATA\opencpn\CHRTLIST.DAT"
			Delete "$APPDATA\opencpn\CHRTLIST.DAT"
			ClearErrors
		${EndIf}
	SectionEnd
	...
	Section /o "Delete SENC Section" SecDeleteSENC
		...
		SetShellVarContext all
		${If} ${FileExists} "$APPDATA\opencpn\SENC\*.*"
			RMDir /r "$APPDATA\opencpn\SENC"
			ClearErrors
		${EndIf}
	SectionEnd
	...
	Section /o "Delete SENC Section" SecDeleteCM93
		;# Delete SENC directory
		SetShellVarContext all
		${If} ${FileExists} "$APPDATA\opencpn\cm93\*.*"
			RMDir /r "$APPDATA\opencpn\cm93"
			ClearErrors
		${EndIf}
	SectionEnd
	...
	Section /o "Delete Navobject Section" SecDeleteNavobjFile
		...
		SetShellVarContext all
		${If} ${FileExists} "$APPDATA\opencpn\navobj.xml"
			Delete "$APPDATA\opencpn\navobj.xml"
			ClearErrors
		${EndIf}
	SectionEnd
	...
	Section /o "Delete Log File Section" SecDeleteLogFile
		...
		SetShellVarContext all
		${If} ${FileExists} "$APPDATA\opencpn\opencpn.log"
			Delete "$APPDATA\opencpn\opencpn.log"
			ClearErrors
		${EndIf}
	SectionEnd
SectionGroupEnd
...
SectionGroup /e "!Shortcuts Section Group" SecGrpShortcuts
	Section "Startmenu Shortcut Section" SecShortcutStartmenu
		...
		!insertmacro MUI_STARTMENU_WRITE_BEGIN Application
			...
			CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\OpenCPN 3.3.2303.lnk" "$INSTDIR\opencpn.exe"
			CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\OpenCPN 3.3.2303 - No OpenGL.lnk" "$INSTDIR\opencpn.exe" "/no_opengl" 
			CreateDirectory "$SMPROGRAMS\$STARTMENU_FOLDER\$(Uninstall_Folder_Name)"
			CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\$(Uninstall_Folder_Name)\$(Uninstall_Link).lnk" "$INSTDIR\Uninstall 3.3.2303.exe"
		!insertmacro MUI_STARTMENU_WRITE_END
	SectionEnd

	Section "Desktop Shortcut Section" SecShortcutDesktop
		SetShellVarContext all
		CreateShortCut "$DESKTOP\OpenCPN 3.3.2303.lnk" "$INSTDIR\opencpn.exe"
	SectionEnd
SectionGroupEnd
...
Section "-Install Section" SecInstall
	...
	ExecWait '"$INSTDIR\vc10\vcredist_x86.exe" /q /norestart' $0
; Remove folder
  RMDir /r "$INSTDIR\vc10"
	...
	;# Create uninstaller
	WriteUninstaller "$INSTDIR\Uninstall 3.3.2303.exe"
	...
	WriteRegStr SHCTX "${REG_KEY_UN}" "DisplayName" "OpenCPN 3.3.2303"
	WriteRegStr SHCTX "${REG_KEY_UN}" "DisplayVersion" "3.3.2303"
	...
	WriteRegStr SHCTX "${REG_KEY_UN}" "UninstallString" "$\"$INSTDIR\Uninstall 3.3.2303.exe$\""
	WriteRegStr SHCTX "${REG_KEY_UN}" "QuietUninstallString" "$\"$INSTDIR\Uninstall 3.3.2303.exe /S$\""
	WriteRegStr SHCTX "${REG_KEY_UN}" "ConfigLocation" "$APPDATA\opencpn"
	WriteRegStr SHCTX "${REG_KEY_UN}" "DisplayIcon" "$INSTDIR\opencpn.exe"
	WriteRegStr SHCTX "${REG_KEY_UN}" "Publisher" "opencpn.org"
	...
	${IfNot} ${FileExists} "$APPDATA\opencpn\*.*"
		CreateDirectory "$APPDATA\opencpn"
		Sleep 250
	${EndIf}
	AccessControlW::GrantOnFile "$APPDATA\opencpn" "(BU)" "GenericRead + GenericExecute + GenericWrite + Delete + FullAccess"
...
Section /o "-un.Backup Config to RegKey" UnSecBackupConfigToRegKey
	...
	${If} ${FileExists} "$APPDATA\opencpn\opencpn.ini"
		...
		loop:
			IntOp $2 $2 + 1
			ReadINIStr $1 "$APPDATA\opencpn\opencpn.ini" "ChartDirectories" "ChartDir$2"
			StrCmp $1 "" cont
			${WordReplace} "$1" "\\" "\" "+" $3
			${unExplode} $4 "^" "$3"
			Pop $5
		...
		ReadINIStr $0 "$APPDATA\opencpn\opencpn.ini" "Settings" "Locale"
		StrCmp $0 "" +2 0
...
Section "-un.Uninstall" UnSecUninstall
	...
	;# Delete subdirs and files in INSTDIR
		...
		Delete "$INSTDIR\install.log"
		  Delete "$INSTDIR\CrashSender1401.exe"
  Delete "$INSTDIR\sounds\1bells.wav"
  Delete "$INSTDIR\sounds\README.bells"
  Delete "$INSTDIR\sounds\2bells.wav"
  Delete "$INSTDIR\vc10\vcredist_x86.exe"
  Delete "$INSTDIR\gshhs\wdb_borders_c.b"
  Delete "$INSTDIR\gshhs\poly-c-1.dat"
  Delete "$INSTDIR\gshhs\wdb_rivers_c.b"
  Delete "$INSTDIR\opencpn.exe"
  Delete "$INSTDIR\wxmsw28u_html_vc_custom.dll"
  Delete "$INSTDIR\plugins\dashboard_pi.dll"
  Delete "$INSTDIR\plugins\grib_pi.dll"
  Delete "$INSTDIR\wxmsw28u_gl_vc_custom.dll"
  Delete "$INSTDIR\wxbase28u_net_vc_custom.dll"
  Delete "$INSTDIR\doc\readme"
  Delete "$INSTDIR\doc\help_en_US.html"
  Delete "$INSTDIR\doc\help_web.html"
  Delete "$INSTDIR\doc\images\own5.png"
  Delete "$INSTDIR\doc\images\mark10.png"
  Delete "$INSTDIR\doc\images\dsc-list.png"
  Delete "$INSTDIR\doc\images\qtvlm1.png"
  Delete "$INSTDIR\doc\images\SART3.png"
  Delete "$INSTDIR\doc\images\5.png"
  Delete "$INSTDIR\doc\images\mark7.png"
  Delete "$INSTDIR\doc\images\layer-prop1.png"
  Delete "$INSTDIR\doc\images\ais-yellow.png"
  Delete "$INSTDIR\doc\images\hidden-quilt.png"
  Delete "$INSTDIR\doc\images\base_vector_quilt.png"
  Delete "$INSTDIR\doc\images\hotkeys2.gif"
  Delete "$INSTDIR\doc\images\cm93.png"
  Delete "$INSTDIR\doc\images\options-charts-load-1.png"
  Delete "$INSTDIR\doc\images\aton_off.png"
  Delete "$INSTDIR\doc\images\DeviceManager-1.jpg"
  Delete "$INSTDIR\doc\images\orig2.png"
  Delete "$INSTDIR\doc\images\out12.png"
  Delete "$INSTDIR\doc\images\multileg.png"
  Delete "$INSTDIR\doc\images\4.png"
  Delete "$INSTDIR\doc\images\gpsg_buddy.jpg"
  Delete "$INSTDIR\doc\images\text.png"
  Delete "$INSTDIR\doc\images\ifeahche_1.png"
  Delete "$INSTDIR\doc\images\composite.png"
  Delete "$INSTDIR\doc\images\out22.png"
  Delete "$INSTDIR\doc\images\dash-drag.png"
  Delete "$INSTDIR\doc\images\portable.png"
  Delete "$INSTDIR\doc\images\wpt_focus2.png"
  Delete "$INSTDIR\doc\images\routing6.png"
  Delete "$INSTDIR\doc\images\gps1.png"
  Delete "$INSTDIR\doc\images\hdg-cog.png"
  Delete "$INSTDIR\doc\images\gribplug.png"
  Delete "$INSTDIR\doc\images\dash-select-instr_0.png"
  Delete "$INSTDIR\doc\images\help_1.png"
  Delete "$INSTDIR\doc\images\gps3.png"
  Delete "$INSTDIR\doc\images\incwpt1.png"
  Delete "$INSTDIR\doc\images\mad-track.png"
  Delete "$INSTDIR\doc\images\gc6.png"
  Delete "$INSTDIR\doc\images\mad-track2.png"
  Delete "$INSTDIR\doc\images\meta.png"
  Delete "$INSTDIR\doc\images\sentence-filters1.png"
  Delete "$INSTDIR\doc\images\route_manager.png"
  Delete "$INSTDIR\doc\images\Tide7.png"
  Delete "$INSTDIR\doc\images\moored.png"
  Delete "$INSTDIR\doc\images\north-up.png"
  Delete "$INSTDIR\doc\images\jman-tool.png"
  Delete "$INSTDIR\doc\images\orig1.png"
  Delete "$INSTDIR\doc\images\WH2.png"
  Delete "$INSTDIR\doc\images\orig.png"
  Delete "$INSTDIR\doc\images\ais-grey.png"
  Delete "$INSTDIR\doc\images\incwpt4.png"
  Delete "$INSTDIR\doc\images\in2.png"
  Delete "$INSTDIR\doc\images\About32.png"
  Delete "$INSTDIR\doc\images\options-Ships6.png"
  Delete "$INSTDIR\doc\images\help_002.png"
  Delete "$INSTDIR\doc\images\dash-set.png"
  Delete "$INSTDIR\doc\images\Con-net2.png"
  Delete "$INSTDIR\doc\images\TS1.png"
  Delete "$INSTDIR\doc\images\options-Ships4.png"
  Delete "$INSTDIR\doc\images\vecoverz.png"
  Delete "$INSTDIR\doc\images\9.png"
  Delete "$INSTDIR\doc\images\buttons-select.png"
  Delete "$INSTDIR\doc\images\restrman_1.png"
  Delete "$INSTDIR\doc\images\own_vector.png"
  Delete "$INSTDIR\doc\images\polyconic.png"
  Delete "$INSTDIR\doc\images\toolbar-2.png"
  Delete "$INSTDIR\doc\images\Route-Properties2.png"
  Delete "$INSTDIR\doc\images\base.png"
  Delete "$INSTDIR\doc\images\SARTalarm.png"
  Delete "$INSTDIR\doc\images\ais-green.png"
  Delete "$INSTDIR\doc\images\stXhole1.png"
  Delete "$INSTDIR\doc\images\dashboard-plugin32.png"
  Delete "$INSTDIR\doc\images\Tide4.png"
  Delete "$INSTDIR\doc\images\inland-blue.png"
  Delete "$INSTDIR\doc\images\ChartGroups322.png"
  Delete "$INSTDIR\doc\images\options-charts-groups-1.png"
  Delete "$INSTDIR\doc\images\layer_route1.png"
  Delete "$INSTDIR\doc\images\Anch2.png"
  Delete "$INSTDIR\doc\images\anegpass.png"
  Delete "$INSTDIR\doc\images\dh.png"
  Delete "$INSTDIR\doc\images\incwpt8.png"
  Delete "$INSTDIR\doc\images\ais-lost.png"
  Delete "$INSTDIR\doc\images\swlook.png"
  Delete "$INSTDIR\doc\images\Anch3.png"
  Delete "$INSTDIR\doc\images\nogl.png"
  Delete "$INSTDIR\doc\images\bramb2.png"
  Delete "$INSTDIR\doc\images\options-charts-tides-2.png"
  Delete "$INSTDIR\doc\images\base_raster_single.png"
  Delete "$INSTDIR\doc\images\plan2.png"
  Delete "$INSTDIR\doc\images\bud-icon.png"
  Delete "$INSTDIR\doc\images\options-plugins.png"
  Delete "$INSTDIR\doc\images\ais2.png"
  Delete "$INSTDIR\doc\images\highlight.png"
  Delete "$INSTDIR\doc\images\Routing3.png"
  Delete "$INSTDIR\doc\images\last-button.png"
  Delete "$INSTDIR\doc\images\options-user2.png"
  Delete "$INSTDIR\doc\images\Tide6.png"
  Delete "$INSTDIR\doc\images\vector-light.png"
  Delete "$INSTDIR\doc\images\charts_MT.png"
  Delete "$INSTDIR\doc\images\AISTargetQuery.png"
  Delete "$INSTDIR\doc\images\gpsd-con1.png"
  Delete "$INSTDIR\doc\images\Routing1.png"
  Delete "$INSTDIR\doc\images\ais_supressed.png"
  Delete "$INSTDIR\doc\images\Anch5.png"
  Delete "$INSTDIR\doc\images\settings_003.png"
  Delete "$INSTDIR\doc\images\offsets4.png"
  Delete "$INSTDIR\doc\images\composite1.png"
  Delete "$INSTDIR\doc\images\skewed32.png"
  Delete "$INSTDIR\doc\images\zoomout.png"
  Delete "$INSTDIR\doc\images\ais-alarm.png"
  Delete "$INSTDIR\doc\images\track-prop1.png"
  Delete "$INSTDIR\doc\images\dim1.png"
  Delete "$INSTDIR\doc\images\options-Ships3.png"
  Delete "$INSTDIR\doc\images\tmerc.png"
  Delete "$INSTDIR\doc\images\Anch8.png"
  Delete "$INSTDIR\doc\images\real-size.png"
  Delete "$INSTDIR\doc\images\dsc-ok.png"
  Delete "$INSTDIR\doc\images\own4.png"
  Delete "$INSTDIR\doc\images\printroute.png"
  Delete "$INSTDIR\doc\images\Route23.png"
  Delete "$INSTDIR\doc\images\grib.png"
  Delete "$INSTDIR\doc\images\Chartbar322.png"
  Delete "$INSTDIR\doc\images\currents_box.png"
  Delete "$INSTDIR\doc\images\Tide2.png"
  Delete "$INSTDIR\doc\images\auto-follow_0.png"
  Delete "$INSTDIR\doc\images\jman-flat-tool.png"
  Delete "$INSTDIR\doc\images\europe1.png"
  Delete "$INSTDIR\doc\images\layer-prop.png"
  Delete "$INSTDIR\doc\images\font.png"
  Delete "$INSTDIR\doc\images\skewed.png"
  Delete "$INSTDIR\doc\images\base_raster_quilt.png"
  Delete "$INSTDIR\doc\images\Anch6.png"
  Delete "$INSTDIR\doc\images\nsplattform.png"
  Delete "$INSTDIR\doc\images\pencil2.png"
  Delete "$INSTDIR\doc\images\track_off.png"
  Delete "$INSTDIR\doc\images\track-prop.png"
  Delete "$INSTDIR\doc\images\aground.png"
  Delete "$INSTDIR\doc\images\gc1.png"
  Delete "$INSTDIR\doc\images\Anch9.png"
  Delete "$INSTDIR\doc\images\2624.png"
  Delete "$INSTDIR\doc\images\Dash24.png"
  Delete "$INSTDIR\doc\images\6.png"
  Delete "$INSTDIR\doc\images\Quilt3.png"
  Delete "$INSTDIR\doc\images\default-mark.png"
  Delete "$INSTDIR\doc\images\explain.png"
  Delete "$INSTDIR\doc\images\Tide-interference.png"
  Delete "$INSTDIR\doc\images\out21.png"
  Delete "$INSTDIR\doc\images\gpsGrn.png"
  Delete "$INSTDIR\doc\images\sentence-filters2.png"
  Delete "$INSTDIR\doc\images\trail.png"
  Delete "$INSTDIR\doc\images\Mob1.png"
  Delete "$INSTDIR\doc\images\status-bar.png"
  Delete "$INSTDIR\doc\images\SART1.png"
  Delete "$INSTDIR\doc\images\WH1.png"
  Delete "$INSTDIR\doc\images\8.png"
  Delete "$INSTDIR\doc\images\gc3.png"
  Delete "$INSTDIR\doc\images\area-query.png"
  Delete "$INSTDIR\doc\images\ship_red.png"
  Delete "$INSTDIR\doc\images\Chartbar32.png"
  Delete "$INSTDIR\doc\images\offsets5.png"
  Delete "$INSTDIR\doc\images\initial-vector.png"
  Delete "$INSTDIR\doc\images\Ships_aistargets.png"
  Delete "$INSTDIR\doc\images\Anch4.png"
  Delete "$INSTDIR\doc\images\vectorSettings.png"
  Delete "$INSTDIR\doc\images\tide-select1.png"
  Delete "$INSTDIR\doc\images\filtering.png"
  Delete "$INSTDIR\doc\images\pencil.png"
  Delete "$INSTDIR\doc\images\objq.png"
  Delete "$INSTDIR\doc\images\dsc-not-ok.png"
  Delete "$INSTDIR\doc\images\europe3.png"
  Delete "$INSTDIR\doc\images\options-Ships2.png"
  Delete "$INSTDIR\doc\images\3.png"
  Delete "$INSTDIR\doc\images\basestn.png"
  Delete "$INSTDIR\doc\images\add-Chart.png"
  Delete "$INSTDIR\doc\images\out11.png"
  Delete "$INSTDIR\doc\images\track-pressed.png"
  Delete "$INSTDIR\doc\images\colscheme.png"
  Delete "$INSTDIR\doc\images\help.png"
  Delete "$INSTDIR\doc\images\rollover.png"
  Delete "$INSTDIR\doc\images\dataconnections4.png"
  Delete "$INSTDIR\doc\images\scout.png"
  Delete "$INSTDIR\doc\images\aisfocus.png"
  Delete "$INSTDIR\doc\images\font1.png"
  Delete "$INSTDIR\doc\images\offsets6.png"
  Delete "$INSTDIR\doc\images\print.png"
  Delete "$INSTDIR\doc\images\TS2.png"
  Delete "$INSTDIR\doc\images\gc8.png"
  Delete "$INSTDIR\doc\images\cm93grp.png"
  Delete "$INSTDIR\doc\images\havannah-ext.png"
  Delete "$INSTDIR\doc\images\platquery.png"
  Delete "$INSTDIR\doc\images\ToolBAR22.png"
  Delete "$INSTDIR\doc\images\Routing8.png"
  Delete "$INSTDIR\doc\images\options-display.png"
  Delete "$INSTDIR\doc\images\bramb.png"
  Delete "$INSTDIR\doc\images\Wpt21.png"
  Delete "$INSTDIR\doc\images\mark3.png"
  Delete "$INSTDIR\doc\images\mark8.png"
  Delete "$INSTDIR\doc\images\rasteroverz.png"
  Delete "$INSTDIR\doc\images\cur1_0.png"
  Delete "$INSTDIR\doc\images\route_focus3.png"
  Delete "$INSTDIR\doc\images\in22.png"
  Delete "$INSTDIR\doc\images\current1.png"
  Delete "$INSTDIR\doc\images\Mob2.png"
  Delete "$INSTDIR\doc\images\rc_active-route.png"
  Delete "$INSTDIR\doc\images\route_002.png"
  Delete "$INSTDIR\doc\images\settings_002.png"
  Delete "$INSTDIR\doc\images\in1.png"
  Delete "$INSTDIR\doc\images\incwpt2.png"
  Delete "$INSTDIR\doc\images\BGchart.png"
  Delete "$INSTDIR\doc\images\7.png"
  Delete "$INSTDIR\doc\images\Routing5.png"
  Delete "$INSTDIR\doc\images\displayspeed.png"
  Delete "$INSTDIR\doc\images\0.png"
  Delete "$INSTDIR\doc\images\ais_alive_active.png"
  Delete "$INSTDIR\doc\images\vdr-icons.png"
  Delete "$INSTDIR\doc\images\VDR-AIS_Simulation-2.png"
  Delete "$INSTDIR\doc\images\rc_wpt-del-mv.png"
  Delete "$INSTDIR\doc\images\add-Chart1.png"
  Delete "$INSTDIR\doc\images\brazil.png"
  Delete "$INSTDIR\doc\images\MOB-choice.png"
  Delete "$INSTDIR\doc\images\grib_0.png"
  Delete "$INSTDIR\doc\images\offsets2.png"
  Delete "$INSTDIR\doc\images\anch.png"
  Delete "$INSTDIR\doc\images\Mob.png"
  Delete "$INSTDIR\doc\images\TargetCPA.png"
  Delete "$INSTDIR\doc\images\10_1mile.png"
  Delete "$INSTDIR\doc\images\Route-Properties.png"
  Delete "$INSTDIR\doc\images\gc-comp.png"
  Delete "$INSTDIR\doc\images\ais_alarm.png"
  Delete "$INSTDIR\doc\images\classB.png"
  Delete "$INSTDIR\doc\images\dyngnaset.png"
  Delete "$INSTDIR\doc\images\AIS-list.png"
  Delete "$INSTDIR\doc\images\bramb1.png"
  Delete "$INSTDIR\doc\images\ais-red.png"
  Delete "$INSTDIR\doc\images\dash-big-fonts.png"
  Delete "$INSTDIR\doc\images\zoomin.png"
  Delete "$INSTDIR\doc\images\folderoptions.png"
  Delete "$INSTDIR\doc\images\Routing7.png"
  Delete "$INSTDIR\doc\images\Route-Properties1.png"
  Delete "$INSTDIR\doc\images\mark9.png"
  Delete "$INSTDIR\doc\images\Routing2.png"
  Delete "$INSTDIR\doc\images\aa-mob.png"
  Delete "$INSTDIR\doc\images\grig-plug.png"
  Delete "$INSTDIR\doc\images\plan5.png"
  Delete "$INSTDIR\doc\images\mark4.png"
  Delete "$INSTDIR\doc\images\lockwpt.png"
  Delete "$INSTDIR\doc\images\layer22.png"
  Delete "$INSTDIR\doc\images\mark6.png"
  Delete "$INSTDIR\doc\images\Tide1.png"
  Delete "$INSTDIR\doc\images\activeroutewpt.png"
  Delete "$INSTDIR\doc\images\ais-moor.png"
  Delete "$INSTDIR\doc\images\WH.png"
  Delete "$INSTDIR\doc\images\sart-alarm2.png"
  Delete "$INSTDIR\doc\images\grib2.png"
  Delete "$INSTDIR\doc\images\route_plugin.png"
  Delete "$INSTDIR\doc\images\2.png"
  Delete "$INSTDIR\doc\images\scin.png"
  Delete "$INSTDIR\doc\images\ais11.png"
  Delete "$INSTDIR\doc\images\current.png"
  Delete "$INSTDIR\doc\images\dsc_distress.jpg"
  Delete "$INSTDIR\doc\images\route.png"
  Delete "$INSTDIR\doc\images\base_vectot_single1.png"
  Delete "$INSTDIR\doc\images\mob_btn.png"
  Delete "$INSTDIR\doc\images\course-up.png"
  Delete "$INSTDIR\doc\images\aton_std.png"
  Delete "$INSTDIR\doc\images\Quilt1.png"
  Delete "$INSTDIR\doc\images\layer-track2.png"
  Delete "$INSTDIR\doc\images\dashn3.png"
  Delete "$INSTDIR\doc\images\route-legs.png"
  Delete "$INSTDIR\doc\images\tracks.png"
  Delete "$INSTDIR\doc\images\bluesquare.png"
  Delete "$INSTDIR\doc\images\opencpn.png"
  Delete "$INSTDIR\doc\images\Quilt4.png"
  Delete "$INSTDIR\doc\images\trad-tool.png"
  Delete "$INSTDIR\doc\images\settings.png"
  Delete "$INSTDIR\doc\images\havannah-noext.png"
  Delete "$INSTDIR\doc\images\Dashprop322.png"
  Delete "$INSTDIR\doc\images\tide-select2.png"
  Delete "$INSTDIR\doc\images\options-ships-ais.png"
  Delete "$INSTDIR\doc\images\incwpt3.png"
  Delete "$INSTDIR\doc\images\mark2.png"
  Delete "$INSTDIR\doc\images\gpsRed.png"
  Delete "$INSTDIR\doc\images\gc4.png"
  Delete "$INSTDIR\doc\images\incwpt7.png"
  Delete "$INSTDIR\doc\images\sentence-filters.png"
  Delete "$INSTDIR\doc\images\dashboard-docked32.png"
  Delete "$INSTDIR\doc\images\in11.png"
  Delete "$INSTDIR\doc\images\gc7.png"
  Delete "$INSTDIR\doc\images\quilt-ref.png"
  Delete "$INSTDIR\doc\images\sounding.png"
  Delete "$INSTDIR\doc\images\action-filter.png"
  Delete "$INSTDIR\doc\images\tide.png"
  Delete "$INSTDIR\doc\images\ChartGroups321.png"
  Delete "$INSTDIR\doc\images\incwpt5.png"
  Delete "$INSTDIR\doc\images\charts-tides.png"
  Delete "$INSTDIR\doc\images\options-Ships1.png"
  Delete "$INSTDIR\doc\images\ship_grey.png"
  Delete "$INSTDIR\doc\images\hsc.png"
  Delete "$INSTDIR\doc\images\fishing.png"
  Delete "$INSTDIR\doc\images\incwpt6.png"
  Delete "$INSTDIR\doc\images\route-legs1.png"
  Delete "$INSTDIR\doc\images\dsc-ok2.png"
  Delete "$INSTDIR\doc\images\Route-rollover.png"
  Delete "$INSTDIR\doc\images\not_participate.png"
  Delete "$INSTDIR\doc\images\Reduce-Data.png"
  Delete "$INSTDIR\doc\images\dashboard32.png"
  Delete "$INSTDIR\doc\images\plan1.png"
  Delete "$INSTDIR\doc\images\Tide5.png"
  Delete "$INSTDIR\doc\images\ais_disabled.png"
  Delete "$INSTDIR\doc\images\layers-show2.png"
  Delete "$INSTDIR\doc\images\1_2.png"
  Delete "$INSTDIR\doc\images\notundcom.png"
  Delete "$INSTDIR\doc\images\SART5.png"
  Delete "$INSTDIR\doc\images\gps2.png"
  Delete "$INSTDIR\doc\images\Debug_win.png"
  Delete "$INSTDIR\doc\images\Quilt2.png"
  Delete "$INSTDIR\doc\images\Reduce-Data1.png"
  Delete "$INSTDIR\doc\images\48px-Under_construction_icon-blue.png"
  Delete "$INSTDIR\doc\images\Prolific.jpg"
  Delete "$INSTDIR\doc\images\seg.png"
  Delete "$INSTDIR\doc\images\vdr-control.png"
  Delete "$INSTDIR\doc\images\course-up_0.png"
  Delete "$INSTDIR\doc\images\incwpt.png"
  Delete "$INSTDIR\doc\images\route-button.png"
  Delete "$INSTDIR\doc\images\Route21.png"
  Delete "$INSTDIR\doc\images\gl.png"
  Delete "$INSTDIR\doc\images\SART4.png"
  Delete "$INSTDIR\doc\images\aton_virt.png"
  Delete "$INSTDIR\doc\images\options-Ships5.png"
  Delete "$INSTDIR\doc\images\ais.png"
  Delete "$INSTDIR\wxbase28u_vc_custom.dll"
  Delete "$INSTDIR\wxmsw28u_aui_vc_custom.dll"
  Delete "$INSTDIR\CrashRpt1401.dll"
  Delete "$INSTDIR\wxbase28u_xml_vc_custom.dll"
  Delete "$INSTDIR\ocpn_gltest1.exe"
  Delete "$INSTDIR\s57data\s57expectedinput.csv"
  Delete "$INSTDIR\s57data\rastersymbols-dusk.png"
  Delete "$INSTDIR\s57data\S52RAZDS.RLE"
  Delete "$INSTDIR\s57data\attdecode.csv"
  Delete "$INSTDIR\s57data\s57attributes.csv"
  Delete "$INSTDIR\s57data\rastersymbols-day.png"
  Delete "$INSTDIR\s57data\chartsymbols.xml"
  Delete "$INSTDIR\s57data\rastersymbols-dark.png"
  Delete "$INSTDIR\s57data\s57objectclasses.csv"
  Delete "$INSTDIR\dbghelp.dll"
  Delete "$INSTDIR\license.txt"
  Delete "$INSTDIR\share\locale\vi\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ca@valencia\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\zh_CN\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ar\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\hu_HU\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\hu_HU\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\tr\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\nb\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\id\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\nb_NO\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\nb_NO\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\nb_NO\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\pt_BR\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\pt_BR\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\pt_BR\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\pt_BR\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\el_GR\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\el_GR\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\el_GR\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\af\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\fi\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\uk\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\hi\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\pt_PT\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\pt_PT\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\pt_PT\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\de\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\de\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\de\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\de\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\an\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\da\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\da\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\da\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\da\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\es\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\es\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\es\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\es\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\sk\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\tr_TR\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\tr_TR\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\tr_TR\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\ta\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ca_ES\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\ne\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\pt\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\fi_FI\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\fi_FI\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\fi_FI\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\sq\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ms\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\cs\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\cs\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\cs\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\cs\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\zh_TW\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\zh_TW\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\zh_TW\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\zh_TW\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\fr\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\fr\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\fr\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\fr\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\el\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ro\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\sv\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\sv\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\sv\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\sv\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\hu\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\hu\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\it\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\it\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\it\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\it\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\gl_ES\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\gl_ES\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ja\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\et\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\et\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\et\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\lt\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\lv\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ru\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\ru\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\ru\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\ru\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ca\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\pl\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\pl\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\pl\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\pl\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\nl\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\nl\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\nl\LC_MESSAGES\opencpn.mo"
  Delete "$INSTDIR\share\locale\nl\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\eu\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\ko_KR\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\locale\pl_PL\LC_MESSAGES\opencpn-dashboard_pi.mo"
  Delete "$INSTDIR\share\locale\pl_PL\LC_MESSAGES\opencpn-grib_pi.mo"
  Delete "$INSTDIR\share\locale\sl\LC_MESSAGES\wxstd.mo"
  Delete "$INSTDIR\share\doc\opencpn\changelog.Debian.gz"
  Delete "$INSTDIR\share\doc\opencpn\copyright"
  Delete "$INSTDIR\uidata\toolicons_journeyman_flat.png"
  Delete "$INSTDIR\uidata\toolicons_traditional.png"
  Delete "$INSTDIR\uidata\styles.xml"
  Delete "$INSTDIR\uidata\toolicons_journeyman.png"
  Delete "$INSTDIR\wxmsw28u_adv_vc_custom.dll"
  Delete "$INSTDIR\crashrpt_lang.ini"
  Delete "$INSTDIR\PrivacyPolicy.txt"
  Delete "$INSTDIR\tcdata\README.harmonics"
  Delete "$INSTDIR\tcdata\HARMONIC"
  Delete "$INSTDIR\tcdata\HARMONIC.IDX"
  ...
  RMDir "$INSTDIR\share\locale\ko_KR\LC_MESSAGES"
  RMDir "$INSTDIR\share\locale\ko_KR"
  ...
  RMDir "$INSTDIR\share\locale"
  RMDir "$INSTDIR\share\doc\opencpn"
  RMDir "$INSTDIR\share\doc"
  RMDir "$INSTDIR\share"
  RMDir "$INSTDIR\uidata"
  RMDir "$INSTDIR\tcdata"
...
Function .onInit
	...
	${IfNot} ${UAC_IsInnerInstance}
		System::Call 'kernel32::CreateMutexW(i 0, i 0, t "ocpn_inst_3.3.2303") i .r1 ?e'
		Pop $R0
		StrCmp $R0 0 +3
...
##########  from here we can be sure to run with admin privileges  ##########
	...
	${StrFilter} "3" "1" "" "" $MAJOR_D
	${StrFilter} "3" "1" "" "" $MINOR_D
	${StrFilter} "2303" "1" "" "" $PATCH_D
...
;# Page_TypeInst_Leave
	Function Page_TypeInst_Leave
		...
		${If} $INSTALL_TYPE == 4
			StrCpy $INSTDIR "$PROGRAMFILES\OpenCPN 3.3.2303"
		${EndIf}
	FunctionEnd
	...
	Function Page_SetConfig
		...
		${IfNot} $CHART_DIR_EMPTY == 1
			...
			${ElseIf} $INI_EXISTS == 1
				...
				loop2:
					IntOp $R2 $R2 + 1
					ReadINIStr $R1 "$APPDATA\opencpn\opencpn.ini" "ChartDirectories" "ChartDir$R2"
					StrCmp $R1 "" cont2
					${WordReplace} "$R1" "\\" "\" "+" $R3
					${Explode} $R4 "^" "$R3"
					Pop $R5
...
Function Check_Config_Exists
	...
	${IfNot} ${FileExists} "$APPDATA\opencpn\*.*"
		${If} ${FileExists} "$USERS_APPDATA\opencpn\*.*"
			CreateDirectory "$APPDATA\opencpn"
			Sleep 250
			AccessControlW::GrantOnFile "$APPDATA\opencpn" "(BU)" "GenericRead + GenericExecute + GenericWrite + Delete + FullAccess"
			CopyFiles /SILENT "$USERS_APPDATA\opencpn\*.*" "$APPDATA\opencpn"
		${ElseIf} ${FileExists} "$ADMIN_APPDATA\opencpn\*.*"
			CreateDirectory "$APPDATA\opencpn"
			Sleep 250
			AccessControlW::GrantOnFile "$APPDATA\opencpn" "(BU)" "GenericRead + GenericExecute + GenericWrite + Delete + FullAccess"
			CopyFiles /SILENT "$ADMIN_APPDATA\opencpn\*.*" "$APPDATA\opencpn"
		${EndIf}

		${If} ${FileExists} "$APPDATA\opencpn\opencpn.ini"
			DeleteINISec "$APPDATA\opencpn\opencpn.ini" Directories
		${EndIf}
	${EndIf}
...
Function Users_Appdata_Paths
	...
	${If} ${FileExists} "$2\opencpn\opencpn.ini"
		ReadINIStr $3 "$2\opencpn\opencpn.ini" Directories S57DataLocation
		${WordReplace} "$3" "\\" "\" "+" $4
		${WordFind} "$4" "\" "-2{*" $5
		StrCmp $5 "" 0 +2
		StrCpy $5 "none"
...
Function un.onInit
	...
	${IfNot} ${UAC_IsInnerInstance}
		System::Call 'kernel32::CreateMutexW(i 0, i 0, t "ocpn_uninst_3.3.2303") i .r1 ?e'
		Pop $R0
		StrCmp $R0 0 +3
...
!macro VersionDate_Macro un
	Function ${un}VersionDate
		...
		${If} $0 != $1
			...
			StrCpy $VERSION_DATE "3.3.$Patch_Date"
		...
		${Else}
			...
			${EndIf}
			${VersionConvert} $Version_Unified "" $Patch_Date
			StrCpy $VERSION_DATE "3.3.$Patch_Date.00"
		${EndIf}
	FunctionEnd
!macroend
...
;# License page
	LicenseLangString license ${LANG_ENGLISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_GERMAN} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_FRENCH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_CZECH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_DANISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_SPANISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_ITALIAN} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_DUTCH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_POLISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_PORTUGUESE} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_PORTUGUESEBR} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_RUSSIAN} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_SWEDISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_FINNISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_NORWEGIAN} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_TRADCHINESE} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
	LicenseLangString license ${LANG_TURKISH} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
        ;LicenseLangString license ${LANG_HUNGARIAN} "P:/working area/opencpn/remote/jongough/OpenCPN/data/license.txt"
...
;#IMPORTANT: Do not end a multi-line with $\"
  ...
  ;Language Dialog
	LangString LangDialog_Title ${LANG_ENGLISH} "OpenCPN 3.3.2303 Installation"
  ...
  ;Un-install Link
	LangString Uninstall_Link ${LANG_ENGLISH} "Uninstall OpenCPN 3.3.2303"
  ...
  ;Version exists
	...
	;Subtitle
    LangString Page_TypeInst_Subtitle_Exists ${LANG_ENGLISH} "OpenCPN 3.3.2303 is already installed.$\r$\nChoose how you want to install OpenCPN."

	;Label
	LangString Label_TypeInst_Reinstall_Text ${LANG_ENGLISH} "OpenCPN 3.3.2303 is already installed on your system. You can reinstall the version now using the previous settings. If you wish to install the program with different settings, cancel installation and un-install the existing version first. Click Next to continue."
  ...
  ;Finish page
	...
	LangString Finish_Link_URL ${LANG_ENGLISH} "http://opencpn.info/?ref=installer&user=$USERS_NAME&lang=$REG_LANG&version=3.3.2303"
  ...
  ;added on 2011-06-13
	LangString ^NameDA ${LANG_ENGLISH} "OpenCPN Version 3.3.2303"
    LangString Installer ${LANG_ENGLISH} "Installer"
