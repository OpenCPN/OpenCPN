...
bool CommDecoder::DecodePGN129029(std::vector<unsigned char> v,
  ...
  if (ParseN2kPGN129029(v, SID, DaysSince1970, SecondsSinceMidnight, Latitude,
    ...
    if ((GNSSmethod == N2kGNSSm_GNSSfix) || (GNSSmethod == N2kGNSSm_DGNSS) ||
        (GNSSmethod == N2kGNSSm_PreciseGNSS)) {
      if (nSatellites > 0) temp_data.n_satellites = nSatellites;
    }
...
bool CommDecoder::DecodeSignalK(std::string s, NavData& temp_data) {
  ...
  if (root.HasMember("updates") && root["updates"].IsArray()) {
    for (rapidjson::Value::ConstValueIterator itr = root["updates"].Begin();
         itr != root["updates"].End(); ++itr) {
