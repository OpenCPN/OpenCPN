...
bool Route::GetPositionAtTime(const wxDateTime &timestamp, double &lat,
  ...
  if (pRoutePointList->GetCount() == 1) {
    RoutePoint *rp = pRoutePointList->GetFirst()->GetData();
    lat = rp->m_lat;
    lon = rp->m_lon;
  ...
  wxRoutePointListNode *node = pRoutePointList->GetFirst();
  RoutePoint *prevPoint = node->GetData();
  node = node->GetNext();

  while (node) {
    RoutePoint *currPoint = node->GetData();
    ...
    if (timestamp > currPoint->GetETA() && !node->GetNext()) {
  ...
  RoutePoint *lastPoint = pRoutePointList->GetLast()->GetData();
  lat = lastPoint->m_lat;
  lon = lastPoint->m_lon;
...
wxDateTime Route::GetTimeAtPosition(double targetLat, double targetLon,
  ...
  if (!m_PlannedDeparture.IsValid() || pRoutePointList->IsEmpty()) {
