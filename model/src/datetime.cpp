...
/**
 * Returns a consistent abbreviated timezone name across all platforms.
 *
 * This function addresses cross-platform differences in timezone formatting:
 * - Windows tends to return full timezone names like "Pacific Standard Time"
 * - UNIX systems typically return abbreviations like "PST"
 *
 * @return Abbreviated timezone name (e.g., "EST", "UTC", "CST", "PDT")
 */
wxString GetTimezoneAbbreviation(wxDateTime date_time) {
  wxDateTime::Tm wxTm = date_time.GetTm();
  std::tm tm = {};
  tm.tm_sec = wxTm.sec;
  tm.tm_min = wxTm.min;
  ...
  wxString tzName = oss.str();

  // If we got a long timezone name (common on Windows)
  // Create an abbreviation by extracting the first letter of each word
  // For example: "Pacific Standard Time" -> "PST"
  if (tzName.Length() > 5) {  // Threshold for "long name"
    wxStringTokenizer tokenizer(tzName, " ");
    wxString abbrev;

    while (tokenizer.HasMoreTokens()) {
      wxString token = tokenizer.GetNextToken();
      if (!token.IsEmpty()) {
        // Add the first character of each word to create the abbreviation
        abbrev += token[0];
      }
    }

    // Only use our generated abbreviation if we actually got one
    if (!abbrev.IsEmpty()) {
      return abbrev;
    }
  }

  // Return the original timezone string if it was already short
  // or if we couldn't generate a good abbreviation
  return tzName;
}
...
wxString toUsrDateTimeFormat(const wxDateTime date_time,
  ...
  if (effective_time_zone == "Local Time") {
    ...
    if (options.show_timezone) {
      ...
      // system. Formatting with the actual timezone (rather than "LOC") makes
      // the labels unambiguous, even if the user changes the timezone settings
      // in the operating system. For example "2021-10-31 01:30:00 EDT" is
      // unambiguous, while "2021-10-31 01:30:00 LOC" is not.
      tzName = GetTimezoneAbbreviation(t);
    }
