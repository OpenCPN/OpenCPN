...
* *Scheduled* runs use background catalog fetches and chart transfers, stepping one
  catalog refresh or one chart download per idle cycle so the UI stays responsive.
* Charts that require manual download in a browser are skipped during
  scheduled runs (logged only).
