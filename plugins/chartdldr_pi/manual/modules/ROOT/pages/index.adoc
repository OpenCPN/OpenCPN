...
* OpenCPN must be running at the scheduled local time.
* Charts that require manual download in a browser are skipped during
  scheduled runs (logged only).
* After the scheduled local time, OpenCPN checks about once per minute whether
  a run is due.
* *Skipped* runs (for example bulk update disabled, no chart sources, or panel
  unavailable) are recorded once for that day and are not retried until the next
  day.
* *Failed* runs (no charts selected, or downloads attempted but none succeeded)
  do not count as completed for that day; OpenCPN will try again on later checks
  the same day.
