...
public:
  ...
  /** Returns false if any Write fails or conf is null. */
  bool Save(wxFileConfig* conf) const;
...
/** Timer intervals for scheduled bulk updates. */
struct ChartDldrScheduleTiming {
  /** How often to check whether a daily scheduled run should start. */
  static constexpr int kCheckIntervalMs = 60 * 1000;
};
