  ...
  while (true) {
    ...
    if (step == ChartDldrBulkChartStepResult::TransferInProgress) {
      if (profile.charts.transfer_poll ==
        ...
        panel.WaitForTransfer(profile,
                              profile.ui.show_download_progress_ui &&
