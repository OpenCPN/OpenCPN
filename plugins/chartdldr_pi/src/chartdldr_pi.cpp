...
int chartdldr_pi::Init(void) {
  ...
  for (const auto &source : m_ChartSources) {
    if (source) {
      ChartDldrRecoverOrphanExtractTrees(source->GetDir());
    }
  }
...
bool chartdldr_pi::DeInit(void) {
  ...
  m_scheduler.InvalidatePendingCallbacks();
  // Detach stops the schedule timer before clearing g_pi.
  m_scheduler.Detach();
  if (g_pi == this) {
    g_pi = nullptr;
  }

  // Tear down the panel (and any in-flight scheduled run) before clearing
  // m_ChartSources. Catalog mutations and full SaveConfig must not run against
  // an already-empty source list during teardown.
...
void chartdldr_pi::OnSetupOptions(void) {
  ...
  m_dldrpanel->RefreshCatalogToolbar();
  m_dldrpanel->Show();
...
void chartdldr_pi::OnCloseToolboxPanel(int page_sel, int ok_apply_cancel) {
  ...
  m_dldrpanel->CancelBulkActivity(ChartDldrBulkCancelScope::OptionsClosed);
  SaveConfig();
  ChartDldrParkDownloaderPanelOnHost(this);
}
...
bool chartdldr_pi::LoadConfig(void) {
  ...
  if (pConf) {
    ...
    ChartDldrScheduleConfig schedule_candidate = m_schedule;
    if (ChartDldrPromoteStalePending(schedule_candidate)) {
      if (SaveScheduleConfig(schedule_candidate)) {
        m_schedule = schedule_candidate;
      } else {
        wxLogWarning(
            "chartdldr_pi: failed to persist interrupted scheduled attempt");
      }
    }
    return true;
  } else
...
bool chartdldr_pi::SaveScheduleConfig(const ChartDldrScheduleConfig &schedule) {
  wxFileConfig *pConf = (wxFileConfig *)m_pconfig;
  if (!pConf) {
    return false;
  }
  pConf->SetPath(_T ( "/Settings/ChartDnldr" ));
  if (schedule.Save(pConf) && pConf->Flush()) {
    return true;
  }
  // wxFileConfig writes mutate its in-memory map before Flush. Restore the
  // live schedule so a later unrelated SaveConfig cannot persist a candidate
  // whose commit failed.
  if (!m_schedule.Save(pConf) || !pConf->Flush()) {
    wxLogError(
        "chartdldr_pi: failed to restore schedule config after write "
        "failure");
  }
  return false;
}
...
void chartdldr_pi::UpdatePrefs(ChartDldrPrefsDlgImpl *dialog) {
  ...
  SaveConfig();
  if (m_dldrpanel) m_dldrpanel->RefreshCatalogToolbar();
  m_scheduler.Restart();
}

void chartdldr_pi::ApplyChartDatabaseUpdate() {
  // Complete-only: applied once after successful downloads. Cancel never
  // applies. UpdateChartDBInplace is best-effort (no host busy API).
...
void chartdldr_pi::AttachDownloaderPanelToOptions(wxScrolledWindow *page) {
  ChartDldrShowDownloaderPanelOnOptions(this, page);
}

bool chartdldr_pi::RequestManualScheduledBulkUpdate() {
  return ChartDldrRequestBulkUpdate(this, ChartDldrBulkRunMode::ScheduledBulk,
...
bool chartdldr_pi::ConfirmInteractiveBulkUpdate(wxWindow *parent) const {
  ...
  message +=
      _("\n\nCharts requiring manual download will open in your browser and be "
        "skipped by the automatic downloader.");
...
ChartDldrBulkRequestInput chartdldr_pi::MakeBulkRequestInput() const {
  ...
  if (m_dldrpanel && m_dldrpanel->Bulk().IsRunActive()) {
