...
void GribRequestSetting::SaveConfig() {
  ...
  if (pConf) {
    ...
    m_SavedZoneSelMode = m_cUseSavedZone->GetValue()    ? SAVED_SELECTION
                         : m_rbCustomSelect->GetValue() ? START_SELECTION
                                                        : AUTO_SELECTION;
...
void GribRequestSetting::InitRequestConfig() {
  ...
  if (pConf) {
    ...
    m_rbCustomSelect->SetValue(m_ZoneSelMode != AUTO_SELECTION);
...
bool GribRequestSetting::MouseEventHook(wxMouseEvent &event) {
  ...
  if (event.LeftDown() && !event.Dragging()) {
    ...
    m_ZoneSelMode = DRAW_SELECTION;
    m_rbCustomSelect->SetValue(true);  // Update the UI checkbox
  }
...
void GribRequestSetting::OnVpChange(PlugIn_ViewPort *vp) {
  ...
  if (m_rbCustomSelect->GetValue()) return;
...
void GribRequestSetting::OnZoneSelectionModeChange(wxCommandEvent &event) {
  ...
  if (event.GetId() == AUTOSELECT) {
    m_rbCustomSelect->SetValue(false);
    ...
    m_ZoneSelMode =
        m_rbCustomSelect->GetValue() ? DRAW_SELECTION : AUTO_SELECTION;
...
double GribRequestSetting::GetMinLat() const {
  if (m_rbCustomSelect && m_rbCustomSelect->GetValue()) {
