...
static bool RecordIsWave(GribRecord *rec) {
  return rec->getDataType() == GRB_HTSGW ||  // Combined significant wave height
         rec->getDataType() == GRB_DIR ||    // Combined wave direction
         rec->getDataType() == GRB_PER ||    // Combined wave period
...
void GribReader::copyMissingWaveRecords() {
  ...
  copyMissingWaveRecords(GRB_HTSGW, LV_GND_SURF, 0);  // Combined sig height
  copyMissingWaveRecords(GRB_DIR, LV_GND_SURF, 0);    // Combined direction
  copyMissingWaveRecords(GRB_PER, LV_GND_SURF, 0);    // Combined period
