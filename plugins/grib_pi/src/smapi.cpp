...
void wxMapiSession::Initialise() {
  ...
  if (bMapiInstalled) {
    ...
    if (m_data->m_hMapi) {
      ...
      m_data->m_lpfnMAPILogoff =
          (LPMAPILOGOFF)GetProcAddress(m_data->m_hMapi, "MAPILogoff");
      m_data->m_lpfnMAPISendMail =
          (LPMAPISENDMAIL)GetProcAddress(m_data->m_hMapi, "MAPISendMail");
      m_data->m_lpfnMAPIResolveName =
          (LPMAPIRESOLVENAME)GetProcAddress(m_data->m_hMapi, "MAPIResolveName");
      m_data->m_lpfnMAPIFreeBuffer =
    ...
    wxLogMessage(
        _("MAIL Error: MAPI32.DLL is not installed or invalid on this "
          "computer!"));
  }
}
...
bool wxMapiSession::Logon(const wxString& sProfileName,
                          const wxString& sPassword, wxWindow* pParentWnd) {
  ...
  if (nProfileLength == 0) {
    ...
    if (pParentWnd) {
      ...
      // No window given, just use the main window of the app as the parent
      // window
  ...
  ULONG nError =
      m_data->m_lpfnMAPILogon(nUIParam, pszProfileName, pszPassword,
...
bool wxMapiSession::Send(wxMailMessage& message) {
  ...
  // Create the MapiMessage structure to match the message parameter send into
  // us
...
#endif
  mapiMessage.nRecipCount = message.m_to.GetCount() + message.m_cc.GetCount() +
                            message.m_bcc.GetCount();
  ...
  /*   //Setup the "CC" recipients
     ...
     if (nTitleSize)
     {
         wxASSERT(nTitleSize == nAttachmentSize); //If you are going to set the
 attachment titles then you must set
  ...
  ULONG nError = m_data->m_lpfnMAPISendMail(m_data->m_hSession, 0, &mapiMessage,
                                            MAPI_DIALOG, 0);
  if (nError == SUCCESS_SUCCESS) {
    bSuccess = TRUE;
    m_data->m_nLastError = SUCCESS_SUCCESS;
...
#ifdef UNICODE
  for (i = 0; i < nRecipIndex; i++) free(mapiMessage.lpRecips[i].lpszName);
#endif
...
long wxMapiSession::GetLastError() const { return m_data->m_nLastError; }
