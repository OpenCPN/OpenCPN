...
public:
  ...
  // wxJSONValue& operator = ( const void* buff, size_t len ); cannot be
  // declared
  ...
  int AddComment(const wxString& str,
                 int position = wxJSONVALUE_COMMENT_DEFAULT);
  int AddComment(const wxArrayString& comments,
                 int position = wxJSONVALUE_COMMENT_DEFAULT);
  ...
  static wxString MemoryBuffToString(const wxMemoryBuffer& buff,
                                     size_t len = -1);
  static wxString MemoryBuffToString(const void* buff, size_t len,
                                     size_t actualLen = -1);
  static int CompareMemoryBuff(const wxMemoryBuffer& buff1,
