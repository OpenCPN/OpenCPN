...
class GRIBStatproc {
public:
  /** Statistical process type code (0=average, 1=accumulation, 2=maximum,
   * 3=minimum, 4=difference). */
  int proc_code;
  /** Type of time increment for discrete processing (0=none, 1=successive,
   * etc.). */
  int incr_type;
  /** Time unit for time_length (0=minutes, 1=hours, 2=days, etc.). */
  int time_unit;
  /** Length of time range over which statistical processing is done. */
  int time_length;
  /** Time unit for incr_length (0=minutes, 1=hours, 2=days, etc.). */
  int incr_unit;
  /** Time increment between successive statistical processes (0=continuous). */
  int incr_length;
};
      ...
      switch (dataCat) {
        case 0:  // waves
...
#endif
                 // GRIBv2 reference for discipline 10, category 0 (oceanic
                 // products, wave category)
          ...
          switch (dataNum) {
            ...
            case 14:
              // Direction of Combined Wind Waves and Swell, as per the GRIB v2
              // spec. GRIB v1 has no equivalent parameter, so mapping to
              // "Primary Wave Direction" (107) which is semantically similar
              // but technically different.
              // Note: the xygrib server does not support this attribute.
              ret = GRB_DIRPW;
              break;
            case 15:
              // Period of Combined Wind Waves and Swell, as per the GRIB v2
              // spec. GRIB v1 has no equivalent parameter, so mapping to
              // "Primary Wave Period" (108) which is semantically similar but
              // technically different.
              // Note: the xygrib server does not support this attribute.
              ret = GRB_PERPW;
              break;
            // Wind wave parameters - locally generated by current winds
            case 4:
              ret = GRB_WVDIR;
            ...
            case 7:
              ret = GRB_SWDIR;
              break;  // Direction of Swell Waves
            case 8:
              ret = GRB_SWELL;
              break;  // Significant Height of Swell Waves
            case 9:
              ret = GRB_SWPER;
              break;  // Mean Period of Swell Waves
            // Primary wave parameters - dominant wave component
            case 10:
              ret = GRB_DIRPW;
              break;  // Direction of Primary Wave
            case 11:
              ret = GRB_PERPW;
              break;  // Period of Primary Wave

            default:
              // Unknown wave parameter - log for debugging
              grib_debug(
                  "Unknown wave parameter: discipline=10, category=0, "
                  "number=%d",
                  dataNum);
              ret = 255;
              break;
 ...
 * Parameter identification: disc,cat,num where:
 *   disc=0 (Meteorological): cat=0 (Temperature), cat=1 (Moisture), cat=2
 * (Momentum) disc=10 (Oceanographic): cat=0 (Waves), cat=1 (Currents) Common
 * parameters:
 *   (0,0,0)=Temperature, (0,1,7)=Precip rate, (0,2,1)=Wind speed
 *   (10,0,10)=Primary wave dir, (10,0,11)=Primary wave period
 */
            ...
            *p2 = mapStatisticalEndTime(grid);
            ...
            if (grid->md.stat_proc.t[0].incr_length == 0)
              *n_avg = 0;
              ...
              // GRIB1 cannot represent discrete time increments in statistical
              // processing Only continuous processing (incr_length=0) can be
              // mapped to GRIB1 format
              wxLogError(
                  "Unable to map discrete processing to GRIB1 for parameter "
                  "disc=%d,cat=%d,num=%d",
                  grid->disc, grid->md.param_cat, grid->md.param_num);
              return false;
            }
            break;
                      ...
                      *p2 = mapStatisticalEndTime(grid);
                      ...
                      if (grid->md.stat_proc.t[0].incr_length == 0)
                        *n_avg = 0;
                        ...
                        // GRIB1 cannot represent discrete time increments in
                        // statistical processing Only continuous processing
                        // (incr_length=0) can be mapped to GRIB1 format
                        wxLogError(
                            "Unable to map discrete processing to GRIB1 for "
                            "parameter "
                            "disc=%d,cat=%d,num=%d: ",
                            grid->disc, grid->md.param_cat, grid->md.param_num);
                        return false;
...
void GribV2Record::translateDataType() {
  ...
  if (this->knownData) {
    ...
    switch (getDataType()) {
      ...
      case GRB_DIRPW:  // Primary wave direction
      case GRB_PERPW:  // Primary wave period
