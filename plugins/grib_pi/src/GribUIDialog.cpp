...
void GRIBUICtrlBar::OpenFile(bool newestFile) {
  ...
  if (m_bDataPlot[GribOverlaySettings::COMBINED_WAVES] &&
      (m_bGRIBActiveFile->m_GribIdxArray.Index(Idx_DIRPW) != wxNOT_FOUND))
    bconfigOK = true;
  if (m_bDataPlot[GribOverlaySettings::COMBINED_WAVES] &&
  ...
  if (!bconfigOK) {
    for (int i = 0; i < (int)GribOverlaySettings::SETTINGS_COUNT; i++) {
      if (InDataPlot(i)) {
        m_bDataPlot[i] = true;
      }
    }
...
GribTimelineRecordSet *GRIBUICtrlBar::GetTimeLineRecordSet(wxDateTime time) {
  ...
  for (int i = 0; i < Idx_COUNT; i++) {
    ...
    set->SetUnRefGribRecord(i, GribRecord::InterpolatedRecord(
                                   *GR1, *GR2, interp_const, i == Idx_DIRPW));
  }
      ...
      for (unsigned int j = 0; j < m_GribRecordSetArray.GetCount(); j++) {
        if (m_GribRecordSetArray.Item(j).m_Reference_Time == thistime) {
          ...
          switch (pRec->getDataType()) {
            ...
            case GRB_PERPW:
              sigWave = true;
              idx = Idx_PERPW;  // Primary wave period
              break;
            case GRB_DIRPW:
              sigWave = true;
              idx = Idx_DIRPW;  // Primary wave direction
              break;
          ...
          if (m_GribRecordSetArray.Item(j).m_GribRecordPtrArray[idx]) {
            ...
            if (idx == Idx_PRESSURE) {
              ...
              if (sigWave) {
                if (oRec->getDataType() == GRB_DIRPW ||
                    oRec->getDataType() == GRB_PERPW)
                  skip = true;
              }
            }
          }
