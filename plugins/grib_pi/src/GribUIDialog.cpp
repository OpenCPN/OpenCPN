...
void GRIBUICtrlBar::TimelineChanged() {
  ...
  if (!m_InterpolateMode) {
    ...
    m_cRecordForecast->SetString(
        m_Selection_index,
        TToString(time, DT_LONG_DATE_TIME, pPlugIn->GetTimeZone()));  // replace it by the
                                                   // interpolated time label
    m_cRecordForecast->SetStringSelection(TToString(
        time, DT_LONG_DATE_TIME, pPlugIn->GetTimeZone()));  // ensure it's visible in the box
  }
...
void GRIBUICtrlBar::PopulateComboDataList() {
  ...
  for (size_t i = 0; i < rsa->GetCount(); i++) {
    ...
    m_cRecordForecast->Append(TToString(t, DT_LONG_DATE_TIME, pPlugIn->GetTimeZone()));
  }
...
void GRIBUICtrlBar::ComputeBestForecastForNow() {
  ...
  m_cRecordForecast->SetString(
      m_Selection_index,
      TToString(now, DT_LONG_DATE_TIME, pPlugIn->GetTimeZone()));  // write the now date time label
                                                // in the right place in wxChoice
  m_cRecordForecast->SetStringSelection(
      TToString(now, DT_LONG_DATE_TIME, pPlugIn->GetTimeZone()));  // put it in the box
