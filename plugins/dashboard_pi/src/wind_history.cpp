...
void DashboardInstrument_WindDirHistory::SetData(DASH_CAP st, double data,
  ...
  if (st == OCPN_DBP_STC_TWD || st == OCPN_DBP_STC_TWS) {
    if (m_SetNewData < 1) {
      ...
      // start working after we collected 5 records each, as start values for
      // the smoothed curves
      if (m_SpdRecCnt > 5 && m_DirRecCnt > 5) {
        m_IsRunning = true;
        ...
        // set wind angle scale to full +/- 90 degr depending on the real
...
void DashboardInstrument_WindDirHistory::DrawWindDirScale(wxGCDC* dc) {
  ...
  if (!m_IsRunning) {
    ...
    label1 = GetWindDirStr(wxString::Format("%.1f", tempdir));
    // label 2 : 1/4
    tempdir = m_MinWindDir + m_WindDirRange / 4.;
    while (tempdir < 0) tempdir += 360;
    while (tempdir >= 360) tempdir -= 360;
    label2 = GetWindDirStr(wxString::Format("%.1f", tempdir));
    // label 3 : legend for center line
    tempdir = m_MinWindDir + m_WindDirRange / 2;
    while (tempdir < 0) tempdir += 360;
    while (tempdir >= 360) tempdir -= 360;
    label3 = GetWindDirStr(wxString::Format("%.1f", tempdir));
    // label 4 :  3/4
    tempdir = m_MinWindDir + m_WindDirRange * 0.75;
    while (tempdir < 0) tempdir += 360;
    while (tempdir >= 360) tempdir -= 360;
    label4 = GetWindDirStr(wxString::Format("%.1f", tempdir));
    // label 5 : legend for top line
    tempdir = m_MaxWindDir;
    while (tempdir < 0) tempdir += 360;
    while (tempdir >= 360) tempdir -= 360;
    label5 = GetWindDirStr(wxString::Format("%.1f", tempdir));
  }
...
void DashboardInstrument_WindDirHistory::DrawWindSpeedScale(wxGCDC* dc) {
  ...
  if (!m_IsRunning) {
    ...
    */
    ...
    label1.Printf("%.0f %s", m_MaxWindSpdScale, m_WindSpeedUnit.c_str());
    // 3/4 legend
    ...
    if (val1 == 25 || val1 == 75)  // it's a .25 or a .75
      label2.Printf("%.2f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    else if (val1 == 50)
      label2.Printf("%.1f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    else
      label2.Printf("%.0f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    // center legend
    WindSpdScale = m_MaxWindSpdScale / 2.;
    // center line can either have a .0 or .5 value !
    if ((int)(WindSpdScale * 10) % 10 == 5)
      label3.Printf("%.1f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    else
      label3.Printf("%.0f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    ...
    val1 = (int)((WindSpdScale - (int)WindSpdScale) * 100);
    if (val1 == 25 || val1 == 75)
      label4.Printf("%.2f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    else if (val1 == 50)
      label4.Printf("%.1f %s", WindSpdScale, m_WindSpeedUnit.c_str());
    else
      label4.Printf("%.0f %s", WindSpdScale, m_WindSpeedUnit.c_str());

    // bottom legend for min wind, always 0
    label5.Printf("%.0f %s", 0.0, m_WindSpeedUnit.c_str());
  }
  wxFont f;
  if (m_Properties)
    f = m_Properties->m_SmallFont.GetChosenFont();
...
wxString DashboardInstrument_WindDirHistory::GetWindDirStr(wxString WindDir) {
  if (WindDir == "0.0" || WindDir == "360.0")
    ...
    return _("NNE");
  else if (WindDir == "45.0")
    return _("NE");
  else if (WindDir == "67.5")
    return _("ENE");
  else if (WindDir == "90.0")
    return _("E");
  else if (WindDir == "112.5")
    return _("ESE");
  else if (WindDir == "135.0")
    return _("SE");
  else if (WindDir == "157.5")
    return _("SSE");
  else if (WindDir == "180.0")
    return _("S");
  else if (WindDir == "202.5")
    return _("SSW");
  else if (WindDir == "225.0")
    return _("SW");
  else if (WindDir == "247.5")
    return _("WSW");
  else if (WindDir == "270.0")
    return _("W");
  else if (WindDir == "292.5")
    return _("WNW");
  else if (WindDir == "315.0")
    return _("NW");
...
void DashboardInstrument_WindDirHistory::DrawForeground(wxGCDC* dc) {
  ...
  //---------------------------------------------------------------------------------
  dc->SetFont((g_pFontSmall->GetChosenFont()));
  col = wxColour(204, 41, 41, 255);  // red, opaque Set TWD to DataFonf + Color,
                                     // TWS to LabelFOnt + Color
  if (m_Properties) {
    dc->SetFont(m_Properties->m_SmallFont.GetChosenFont());
    dc->SetTextForeground(
        GetColourSchemeFont(m_Properties->m_SmallFont.GetColour()));
    ...
    dc->SetFont(g_pFontSmall->GetChosenFont());
    if (GetColourSchemeFont(g_pFontLabel->GetColour()) ==
  ...
  if (!m_IsRunning)
    ...
    while (dir < 0) dir += 360;
    if (!std::isnan(dir))
      WindAngle = wxString::Format("TWD %3.0f", dir) + DEGREE_SIGN;
    else
      WindAngle = wxString::Format("TWD ---") + DEGREE_SIGN;
  }
  wxFont f;
  if (m_Properties)
    f = m_Properties->m_SmallFont.GetChosenFont();
  ...
  if (m_Properties) {
    ...
    unsigned int r =
        GetColourSchemeFont(m_Properties->m_DataFont.GetColour()).GetRed();
    unsigned int g =
        GetColourSchemeFont(m_Properties->m_DataFont.GetColour()).GetGreen();
    unsigned int b =
        GetColourSchemeFont(m_Properties->m_DataFont.GetColour()).GetBlue();
#else
    unsigned int r =
        GetColourSchemeFont(m_Properties->m_DataFont.GetColour()).Red();
    unsigned int g =
        GetColourSchemeFont(m_Properties->m_DataFont.GetColour()).Green();
    unsigned int b =
        GetColourSchemeFont(m_Properties->m_DataFont.GetColour()).Blue();
#endif
    pen.SetColour(wxColour(r, g, b, 96));  // transparent
  } else {
    if (GetColourSchemeFont(g_pFontData->GetColour()) ==
        GetColourSchemeFont(g_pFontLabel->GetColour()))
  ...
  if (ld > 1) dc->DrawLines(ld, wdDraw);
  ...
  if (!std::isnan(m_WindSpd))
    WindSpeed =
        wxString::Format("TWS %3.1f %s ", m_WindSpd, m_WindSpeedUnit.c_str());
  else
  ...
  wxString statistics =
      wxString::Format(_(" Max %.1f %s since %02d:%02d  Overall %.1f"),
...
#if wxCHECK_VERSION(3, 1, 6)
    unsigned int r =
        GetColourSchemeFont(m_Properties->m_LabelFont.GetColour()).GetRed();
    unsigned int g =
        GetColourSchemeFont(m_Properties->m_LabelFont.GetColour()).GetGreen();
    unsigned int b =
        GetColourSchemeFont(m_Properties->m_LabelFont.GetColour()).GetBlue();
#else
    unsigned int r =
        GetColourSchemeFont(m_Properties->m_LabelFont.GetColour()).Red();
    unsigned int g =
        GetColourSchemeFont(m_Properties->m_LabelFont.GetColour()).Green();
    unsigned int b =
        GetColourSchemeFont(m_Properties->m_LabelFont.GetColour()).Blue();
#endif
    pen.SetColour(wxColour(r, g, b, 96));  // transparent
  } else {
    if (GetColourSchemeFont(g_pFontData->GetColour()) ==
      ...
      unsigned int g =
          GetColourSchemeFont(g_pFontLabel->GetColour()).GetGreen();
  ...
  if (ls > 1) dc->DrawLines(ls, spdDraw);
