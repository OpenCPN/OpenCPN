...
#if wxDEBUG_LEVEL > 0
static const wxChar* traceMask = L"traceReader";
static const wxChar* storeTraceMask = L"StoreComment";
#endif
...
int wxJSONReader::Parse(wxInputStream& is, wxJSONValue* val) {
  ...
  switch (ch) {
    ...
    case '[':
      ...
      break;
    default:
      AddError("Cannot find a start object/array character");
  ...
  do {  // we read until ch < 0
    switch (ch) {
      ...
      case ':':  // key / value separator
        ...
        m_next = 0;
        if (!parent.IsObject()) {
          AddError("\':\' can only used in object's values");
        } else if (!value.IsString()) {
          AddError("\':\' follows a value which is not of type \'string\'");
        } else if (!key.empty()) {
          AddError(
              "\':\' not allowed where a \'name\' string was already "
              "available");
        } else {
  ...
  if (!value.IsValid() && key.empty()) {
    ...
    if (parent.IsObject()) {
      if (!value.IsValid()) {
        ...
        AddError(
            "cannot store the value: \'key\' is missing for JSON object type");
      } else {
      ...
      if (!value.IsValid()) {
        AddError(
            "cannot store the item: \'value\' is missing for JSON array type");
      }
      if (!key.empty()) {
        AddError(
            "cannot store the item: \'key\' (\'%s\') is not permitted in JSON "
            "array type",
            key);
      }
...
void wxJSONReader::AddWarning(int type, const wxString& msg) {
  ...
  wxString err;
  err.Printf("Warning: line %d, col %d - %s", m_lineNo, m_colNo, msg.c_str());
...
int wxJSONReader::SkipComment(wxInputStream& is) {
  ...
  // check if a C-style comment
    ...
    AddError("Strange '/' (did you want to insert a comment?)");
...
int wxJSONReader::ReadString(wxInputStream& is, wxJSONValue& val) {
    ...
    if (ch == '\\') {  // an escape sequence
      ...
      switch (ch) {
        ...
        case 'u':
          ...
          // break;
        default:
          AddError("Unknow escaped character \'\\%c\'", ch);
      }
    } else {
  ...
  if (m_noUtf8) {
    ...
    if (convLen == wxCONV_FAILED) {
      AddError("String value: the UTF-8 stream is invalid");
      s.append("<UTF-8 stream not valid>");
    } else {
#if defined(wxJSON_USE_UNICODE)
  ...
  if (!val.IsValid()) {
    ...
    AddError("String value \'%s\' cannot follow another value", s);
  }
...
int wxJSONReader::ReadValue(wxInputStream& is, int ch, wxJSONValue& val) {
  ...
  if (val.IsValid()) {
    AddError("Value \'%s\' cannot follow a value: \',\' or \':\' missing?", s);
    return nextCh;
  }
  ...
  if (s == "null") {
    ...
    wxLogTrace(traceMask, "(%s) value = NULL", __PRETTY_FUNCTION__);
    return nextCh;
  } else if (s.CmpNoCase("null") == 0) {
    wxLogTrace(traceMask, "(%s) value = NULL", __PRETTY_FUNCTION__);
    AddWarning(wxJSONREADER_CASE, "the \'null\' literal must be lowercase");
    val.SetType(wxJSONTYPE_NULL);
    return nextCh;
  } else if (s == "true") {
    ...
    wxLogTrace(traceMask, "(%s) value = TRUE", __PRETTY_FUNCTION__);
    AddWarning(wxJSONREADER_CASE, "the \'true\' literal must be lowercase");
    val = true;
    return nextCh;
  } else if (s == "false") {
    ...
    val = false;
    return nextCh;
  } else if (s.CmpNoCase("false") == 0) {
    wxLogTrace(traceMask, "(%s) value = FALSE", __PRETTY_FUNCTION__);
    AddWarning(wxJSONREADER_CASE, "the \'false\' literal must be lowercase");
    val = false;
    return nextCh;
  }
  ...
  switch (ch) {
    ...
    case '-':
      ...
      break;
    default:
      AddError("Literal \'%s\' is incorrect (did you forget quotes?)", s);
      return nextCh;
  }
...
int wxJSONReader::AppendUES(wxMemoryBuffer& utf8Buff, const char* uesBuffer) {
  ...
  if (r != 1) {
    AddError("Invalid Unicode Escaped Sequence");
    return -1;
  }
...
int wxJSONReader::ReadMemoryBuff(wxInputStream& is, wxJSONValue& val) {
  ...
  if (!val.IsValid()) {
    ...
    AddError("Memory buffer value cannot follow another value");
  }
