...
int wxJSONReader::DoRead(wxInputStream& is, wxJSONValue& parent) {
  ...
  do {  // we read until ch < 0
    switch (ch) {
      ...
      case '}':
        if (!parent.IsObject()) {
          AddWarning(wxJSONREADER_MISSING, _T("Trying to close an array using the \'}\' (close-object) char" ));
        }
      ...
      case ']':
        if (!parent.IsArray()) {
          ...
          AddWarning(wxJSONREADER_MISSING, _T("Trying to close an object using the \']\' (close-array) char" ));
        }
        StoreValue(ch, key, value, parent);
        m_current = &parent;
        m_next = 0;
...
void wxJSONReader::StoreValue(int ch, const wxString& key, wxJSONValue& value, wxJSONValue& parent) {
  ...
  if (!value.IsValid() && key.empty()) {
    ...
    if (ch == '}' || ch == ']') {
      m_lastStored = 0;
      wxLogTrace(traceMask, _T("(%s) key and value are empty, returning"), __PRETTY_FUNCTION__);
    } else {
    ...
    if (parent.IsObject()) {
      if (!value.IsValid()) {
        ...
        wxLogTrace(traceMask, _T("(%s) adding value to key:%s"), __PRETTY_FUNCTION__, key.c_str());
        parent[key] = value;
...
int wxJSONReader::SkipWhiteSpace(wxInputStream& is) {
  ...
  wxLogTrace(traceMask, _T("(%s) end whitespaces line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
  return ch;
}
...
int wxJSONReader::SkipComment(wxInputStream& is) {
  static const wxChar* warn = _T("Comments may be tolerated in JSON text but they are not part of JSON syntax");
  ...
  wxLogTrace(storeTraceMask, _T("(%s) start comment line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
  ...
  if (ch == '/') {  // C++ comment, read until end-of-line
    ...
    m_comment = wxString::FromUTF8((const char*)utf8Buff.GetData(), utf8Buff.GetDataLen());
  }
  ...
  // check if a C-style comment
    ...
    if (m_noUtf8) {
      m_comment = wxString::From8BitData((const char*)utf8Buff.GetData(), utf8Buff.GetDataLen());
    } else {
      m_comment = wxString::FromUTF8((const char*)utf8Buff.GetData(), utf8Buff.GetDataLen());
    }
  }
  ...
  wxLogTrace(traceMask, _T("(%s) end comment line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
  wxLogTrace(storeTraceMask, _T("(%s) end comment line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
  wxLogTrace(storeTraceMask, _T("(%s) comment=%s"), __PRETTY_FUNCTION__, m_comment.c_str());
  return ch;
}
...
int wxJSONReader::ReadString(wxInputStream& is, wxJSONValue& val) {
  ...
  wxLogTrace(traceMask, _T("(%s) line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
  wxLogTrace(traceMask, _T("(%s) string read=%s"), __PRETTY_FUNCTION__, s.c_str());
  wxLogTrace(traceMask, _T("(%s) value=%s"), __PRETTY_FUNCTION__, val.AsString().c_str());
  ...
  if (!val.IsValid()) {
    ...
    AddWarning(wxJSONREADER_MULTISTRING, _T("Multiline strings are not allowed by JSON syntax"));
...
int wxJSONReader::ReadToken(wxInputStream& is, int ch, wxString& s) {
  ...
  while (nextCh >= 0) {
    switch (nextCh) {
      ...
      case '\b':
        wxLogTrace(traceMask, _T("(%s) line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
        wxLogTrace(traceMask, _T("(%s) token read=%s"), __PRETTY_FUNCTION__, s.c_str());
        return nextCh;
        break;
      default:
  ...
  wxLogTrace(traceMask, _T("(%s) EOF on line=%d col=%d"), __PRETTY_FUNCTION__, m_lineNo, m_colNo);
  wxLogTrace(traceMask, _T("(%s) EOF - token read=%s"), __PRETTY_FUNCTION__, s.c_str());
  return nextCh;
}
...
#if defined(wxJSON_64BIT_INT)
    r = Strtoull(s, &ui64);
    wxLogTrace(traceMask, _T("(%s) convert to wxUint64 result=%d"), __PRETTY_FUNCTION__, r);
    if (r) {
      // store the value
      val = ui64;
      return nextCh;
...
int wxJSONReader::AppendUES(wxMemoryBuffer& utf8Buff, const char* uesBuffer) {
  ...
  wxLogTrace(traceMask, _T("(%s) unicode sequence=%s code=%ld"), __PRETTY_FUNCTION__, uesBuffer, l);
...
void wxJSONReader::StoreComment(const wxJSONValue* parent) {
  ...
  wxLogTrace(storeTraceMask, _T("(%s) m_flags=%d m_commentLine=%d"), __PRETTY_FUNCTION__, m_flags, m_commentLine);
  ...
  if (m_current != 0) {
    wxLogTrace(storeTraceMask, _T("(%s) m_current->lineNo=%d"), __PRETTY_FUNCTION__, m_current->GetLineNo());
    if (m_current->GetLineNo() == m_commentLine) {
      wxLogTrace(storeTraceMask, _T("(%s) comment added to \'m_current\' INLINE"), __PRETTY_FUNCTION__);
  ...
  if (m_next != 0) {
    wxLogTrace(storeTraceMask, _T("(%s) m_next->lineNo=%d"), __PRETTY_FUNCTION__, m_next->GetLineNo());
    if (m_next->GetLineNo() == m_commentLine) {
      wxLogTrace(storeTraceMask, _T("(%s) comment added to \'m_next\' INLINE"), __PRETTY_FUNCTION__);
  ...
  if (m_lastStored != 0) {
    wxLogTrace(storeTraceMask, _T("(%s) m_lastStored->lineNo=%d"), __PRETTY_FUNCTION__, m_lastStored->GetLineNo());
    if (m_lastStored->GetLineNo() == m_commentLine) {
      wxLogTrace(storeTraceMask, _T("(%s) comment added to \'m_lastStored\' INLINE"), __PRETTY_FUNCTION__);
  ...
  if (m_flags & wxJSONREADER_COMMENTS_AFTER) {  // comment AFTER
    if (m_current) {
      if (m_current == parent || !m_current->IsValid()) {
        ...
        wxLogTrace(storeTraceMask, _T("(%s) comment added to m_current (AFTER)"), __PRETTY_FUNCTION__);
        m_current->AddComment(m_comment, wxJSONVALUE_COMMENT_AFTER);
      }
    } else if (m_lastStored) {
      wxLogTrace(storeTraceMask, _T("(%s) comment added to m_lastStored (AFTER)"), __PRETTY_FUNCTION__);
      m_lastStored->AddComment(m_comment, wxJSONVALUE_COMMENT_AFTER);
    } else {
      wxLogTrace(storeTraceMask, _T("(%s) cannot find a value for storing the AFTER comment"), __PRETTY_FUNCTION__);
      AddError(_T("Cannot find a value for storing the comment (flag AFTER)"));
    }
  } else {  // comment BEFORE can only be added to the 'next' value
    if (m_next) {
      wxLogTrace(storeTraceMask, _T("(%s) comment added to m_next (BEFORE)"), __PRETTY_FUNCTION__);
