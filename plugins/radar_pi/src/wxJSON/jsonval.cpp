...
wxJSONRefData::wxJSONRefData() {
  ...
  m_progr = sm_progr;
  ++sm_progr;
  wxLogTrace(traceMask, _T("(%s) JSON refData ctor progr=%d"), __PRETTY_FUNCTION__, m_progr);
#endif
}
...
wxJSONRefData* wxJSONValue::Init(wxJSONType type) {
  ...
  m_progr = sm_progr;
  ++sm_progr;
  wxLogTrace(cowTraceMask, _T("(%s) Init a new object progr=%d"), __PRETTY_FUNCTION__, m_progr);
#endif
  return data;
}
...
// is a copy of 'other' and it has its own progressive
  ...
  m_progr = sm_progr;
  ++sm_progr;
  wxLogTrace(cowTraceMask, _T("(%s) Copy ctor - progr=%d other progr=%d"), __PRETTY_FUNCTION__, m_progr, other.m_progr);
#endif
}
...
wxJSONValue& wxJSONValue::Item(const wxString& key) {
  ...
  wxLogTrace(traceMask, _T("(%s) searching key \'%s' in the actual object"), __PRETTY_FUNCTION__, key.c_str());
  return data->m_valMap[key];
}
...
/*!
 ...
 \endcode
*/
wxJSONValue wxJSONValue::Get(const wxString& key, const wxJSONValue& defaultValue) const {
...
wxString wxJSONValue::GetInfo() const {
  ...
  s.Printf(_T("Object: Progr=%d Type=%s Size=%d comments=%d\n"), data->m_progr, wxJSONValue::TypeToString(data->m_type).c_str(),
...
bool wxJSONValue::IsSameAs(const wxJSONValue& other) const {
  ...
  if (data == otherData) {
    wxLogTrace(compareTraceMask, _T("(%s) objects share the same referenced data - r=TRUE"), __PRETTY_FUNCTION__);
    return true;
  }
    ...
    switch (data->m_type) {
      case wxJSONTYPE_INT:
        if (otherData->m_type == wxJSONTYPE_UINT) {
          ...
          if ((data->m_value.VAL_UINT <= LLONG_MAX) && (data->m_value.VAL_UINT == otherData->m_value.VAL_UINT)) {
            r = true;
          }
  ...
  switch (data->m_type) {
    ...
    case wxJSONTYPE_ARRAY:
      size = Size();
      wxLogTrace(compareTraceMask, _T("(%s) Comparing an array object - size=%d"), __PRETTY_FUNCTION__, size);

      if (size != other.Size()) {
        wxLogTrace(compareTraceMask, _T("(%s) Sizes does not match"), __PRETTY_FUNCTION__);
        return false;
      }
      // compares every element in this object with the element of
      // the same index in the 'other' object
      for (int i = 0; i < size; i++) {
        wxLogTrace(compareTraceMask, _T("(%s) Comparing array element=%d"), __PRETTY_FUNCTION__, i);
    ...
    case wxJSONTYPE_OBJECT:
      size = Size();
      wxLogTrace(compareTraceMask, _T("(%s) Comparing a map obejct - size=%d"), __PRETTY_FUNCTION__, size);

      if (size != other.Size()) {
        wxLogTrace(compareTraceMask, _T("(%s) Comparison failed - sizes does not match"), __PRETTY_FUNCTION__);
        return false;
      }
      ...
      for (it = data->m_valMap.begin(); it != data->m_valMap.end(); it++) {
        ...
        wxLogTrace(compareTraceMask, _T("(%s) Comparing map object - key=%s"), __PRETTY_FUNCTION__, key.c_str());
        wxJSONValue otherVal = other.ItemAt(key);
        bool isSame = it->second.IsSameAs(otherVal);
        if (!isSame) {
          wxLogTrace(compareTraceMask, _T("(%s) Comparison failed for the last object"), __PRETTY_FUNCTION__);
          return false;
        }
      }
      break;
...
wxJSONRefData* wxJSONValue::CloneRefData(const wxJSONRefData* otherData) const {
  ...
  wxLogTrace(cowTraceMask, _T("(%s) CloneRefData() PROGR: other=%d data=%d"), __PRETTY_FUNCTION__, other->GetRefCount(),
             data->GetRefCount());
...
void wxJSONValue::AllocExclusive() {
  ...
  wxASSERT_MSG(m_refData && m_refData->GetRefCount() == 1, _T("wxObject::AllocExclusive() failed."));
}
...
/*/
 ...
 \endcode
*/
wxString wxJSONValue::MemoryBuffToString(const wxMemoryBuffer& buff, size_t len) {
  size_t buffLen = buff.GetDataLen();
...
*/
wxString wxJSONValue::MemoryBuffToString(const void* buff, size_t len, size_t actualLen) {
  wxString s;
